...
use crate::v18::{ActorError, SendError};
...
pub trait Runtime: Primitives + RuntimePolicy {
    ...
    fn create<T: Serialize>(&self, obj: &T) -> Result<(), ActorError> {
        ...
        if root != EMPTY_ARR_CID {
            return Err(
                actor_error_v18!(illegal_state; "failed to create state; expected empty array CID, got: {}", root),
            );
        }
        let new_root = self.store().put_cbor(obj, Code::Blake2b256)
            .map_err(|e| actor_error_v18!(illegal_argument; "failed to write actor state during creation: {}", e.to_string()))?;
    ...
    fn state<T: DeserializeOwned>(&self) -> Result<T, ActorError> {
        Ok(self
            ...
            .map_err(
                |_| actor_error_v18!(illegal_argument; "failed to get actor for Readonly state"),
            )?
