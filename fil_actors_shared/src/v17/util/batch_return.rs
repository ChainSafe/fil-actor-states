...
impl BatchReturn {
    ...
    pub fn of(codes: &[ExitCode]) -> Self {
        let mut g = BatchReturnGen::new(codes.len());
        for code in codes {
            g.add(*code);
        }
        g.generate()
...
pub struct BatchReturnGen {
    ...
    // generate will only work if it has processed all of the expected batch
    expect_count: usize,
}
...
#[cfg(test)]
mod test {
    use crate::v17::util::batch_return::stack;
    use crate::v17::{BatchReturn, FailCode};
    use fvm_shared4::error::ExitCode;
