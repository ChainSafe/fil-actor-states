...
#[macro_export]
macro_rules! actor_error_v17 {
    // Error with only one stringable expression
    ( $code:ident; $msg:expr ) => { $crate::v17::ActorError::$code($msg.to_string()) };

    // String with positional arguments
    ( $code:ident; $msg:literal $(, $ex:expr)+ ) => {
        $crate::v17::ActorError::$code(format!($msg, $($ex,)*))
    };
    ...
    ( $code:ident, $msg:literal $(, $ex:expr)+ ) => {
        $crate::actor_error_v17!($code; $msg $(, $ex)*)
    };
}
