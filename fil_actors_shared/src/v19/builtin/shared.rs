...
use crate::actor_error_v19;
use crate::v19::runtime::builtins::Type;
use crate::v19::{ActorContext, ActorError};
    ...
    if let Some(id) = actor_id {
        ...
        if check_existence {
            rt.get_actor_code_cid(&id)
                .ok_or_else(|| actor_error_v19!(not_found, "no code for address {}", address))?;
        }
        return Ok(id);
    }
    ...
    RT: Runtime,
{
    ...
    match code_cid {
        None => {
            return Err(
                actor_error_v19!(forbidden; "no code for caller {} of method {}", caller, method),
            );
        }
        Some(code_cid) => {
            ...
            match builtin_type {
                None | Some(Type::EVM) => {
                    return Err(
                        actor_error_v19!(forbidden; "caller {} of method {} must be built-in", caller, method),
                    );
                }
    ...
    fn from(s: SendError) -> ActorError {
        match s.0 {
            ...
            fvm_shared4::error::ErrorNumber::NotFound => {
                ...
                actor_error_v19!(unspecified; "receiver not found")
            }
            fvm_shared4::error::ErrorNumber::InsufficientFunds => {
                ...
                actor_error_v19!(insufficient_funds; "not enough funds")
            }
            fvm_shared4::error::ErrorNumber::LimitExceeded => {
                // This means we've exceeded the recursion limit.
                actor_error_v19!(assertion_failed; "recursion limit exceeded")
            }
            ...
            err => {
                ...
                actor_error_v19!(assertion_failed; "unexpected error: {}", err)
            }
        }
    }
}
