...
use fvm_shared4::address::{Address, BLS_PUB_LEN};
...
pub fn apply_code<S: Serialize>(
    ...
    res.ret
        .map_or(RawBytes::default(), |b| RawBytes::new(b.data))
}
...
pub fn apply_ok_implicit<S: Serialize>(
    ...
    let params = params.map(|p| IpldBlock::serialize_cbor(&p).unwrap().unwrap());
    let res = v
        .execute_message_implicit(from, to, value, method, params)
        .unwrap();
    assert_eq!(
        code, res.code,
        "expected code {}, got {} ({})",
...
pub fn get_state<T: DeserializeOwned>(v: &dyn VM, a: &Address) -> Option<T> {
    ...
    v.blockstore()
        .get(&cid)
        .unwrap()
        .map(|slice| fvm_ipld_encoding::from_slice(&slice).unwrap())
}
