...
impl State {
    ...
    pub fn map_addresses_to_id<BS: Blockstore>(
        ...
        let (id, existing) = if let Some(delegated_addr) = delegated_addr {
            ...
            if let Some(existing_id) = map.get(&delegated_key).context_code(
                ExitCode::USR_ILLEGAL_STATE,
                "failed to lookup delegated address",
            )? {
                (*existing_id, true)
            } else {
                let new_id = self.next_id;
                self.next_id += 1;
                map.set(delegated_key, new_id).context_code(
                    ExitCode::USR_ILLEGAL_STATE,
                    "failed to map delegated address",
                )?;
                (new_id, false)
            }
        } else {
