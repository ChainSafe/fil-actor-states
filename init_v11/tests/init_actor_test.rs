...
use fil_actors_runtime_v11::{test_utils::*, EAM_ACTOR_ADDR, EAM_ACTOR_ID};
...
fn call_exec4() {
    ...
    assert_eq!(
        expected_id_addr, resolved_id,
        "f4 address not assigned to the right actor"
    );

    // Try again and expect it to fail with "forbidden".
    let unique_address = Address::new_actor(b"test2");
    rt.new_actor_addr.replace(Some(unique_address));
    let exec_err = exec4_and_verify(
        &rt,
        namespace,
        subaddr,
        *MULTISIG_ACTOR_CODE_ID,
...
fn call_exec4_placeholder() {
    ...
    let expected_id = {
        ...
        let (id, existing) = state
            .map_addresses_to_id(rt.store(), &f4_addr, None)
            .unwrap();
        assert!(!existing);
