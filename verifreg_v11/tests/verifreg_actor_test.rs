...
mod construction {
    ...
    use fil_actors_runtime_v11::test_utils::*;
...
mod verifiers {
    ...
    fn add_verifier_rejects_root() {
        ...
        expect_abort(
            ExitCode::USR_ILLEGAL_ARGUMENT,
            h.add_verifier(&rt, &ROOT_ADDR, &allowance),
        );
        rt.reset();
        h.check_state(&rt);
    }
    ...
    fn remove_verifier_id_address() {
        ...
        rt.id_addresses
            .borrow_mut()
            .insert(verifier_pubkey, *VERIFIER);
...
mod clients {
    ...
    use fil_actors_runtime_v11::test_utils::*;
    use fil_actors_runtime_v11::{DATACAP_TOKEN_ACTOR_ADDR, STORAGE_MARKET_ACTOR_ADDR};
    ...
    fn many_verifiers_and_clients() {
        ...
        h.add_verifier(&rt, &VERIFIER2, &allowance_verifier)
            .unwrap();

        h.add_client(&rt, &VERIFIER, &CLIENT, &allowance_client)
            .unwrap();
        h.add_client(&rt, &VERIFIER, &CLIENT2, &allowance_client)
            .unwrap();

        h.add_client(&rt, &VERIFIER2, &CLIENT3, &allowance_client)
            .unwrap();
        h.add_client(&rt, &VERIFIER2, &CLIENT4, &allowance_client)
            .unwrap();
    ...
    fn resolves_client_address() {
        ...
        h.add_verifier(&rt, &VERIFIER, &allowance_verifier).unwrap();
        h.add_client(&rt, &VERIFIER, &client_pubkey, &allowance_client)
            .unwrap();
    ...
    fn add_verified_client_restricted_correctly() {
        ...
        let params = AddVerifiedClientParams {
            address: *CLIENT,
            allowance: allowance_client.clone(),
        };
...
mod allocs_claims {
    ...
    use fil_actors_runtime_v11::runtime::policy_constants::{
    ...
    use fil_actors_runtime_v11::FailCode;
    use harness::*;
    ...
    fn expire_allocs() {
        ...
        let ret = h
            .remove_expired_allocations(&rt, CLIENT1, vec![id1, id2], 0)
            .unwrap();
        ...
        rt.set_epoch(200);
        let ret = h
            .remove_expired_allocations(&rt, CLIENT2, vec![id1, id2], 0)
            .unwrap();
        assert_eq!(vec![1, 2], ret.considered);
        assert_eq!(
            vec![ExitCode::USR_NOT_FOUND, ExitCode::USR_NOT_FOUND],
            ret.results.codes()
        );
        assert_eq!(DataCap::zero(), ret.datacap_recovered);
        ...
        rt.set_epoch(100);
        let ret = h
            .remove_expired_allocations(&rt, CLIENT1, vec![id1, id2], alloc1.size.0)
            .unwrap();
        assert_eq!(vec![1, 2], ret.considered);
        assert_eq!(
            vec![ExitCode::OK, ExitCode::USR_FORBIDDEN],
            ret.results.codes()
        );
        ...
        rt.set_epoch(200);
        let ret = h
            .remove_expired_allocations(&rt, CLIENT1, vec![id1, id2], alloc2.size.0)
            .unwrap();
        assert_eq!(vec![1, 2], ret.considered);
        assert_eq!(
            vec![ExitCode::USR_NOT_FOUND, ExitCode::OK],
            ret.results.codes()
        );
        ...
        let ret = h
            .remove_expired_allocations(&rt, CLIENT1, vec![id1, id2], total_size)
            .unwrap();
        assert_eq!(vec![1, 2], ret.considered);
        ...
        rt.replace_state(&state_with_allocs);
        let ret = h
            .remove_expired_allocations(&rt, CLIENT1, vec![id1], alloc1.size.0)
            .unwrap();
        ...
        rt.replace_state(&state_with_allocs);
        let ret = h
            .remove_expired_allocations(&rt, CLIENT1, vec![], 0)
            .unwrap();
        ...
        rt.set_epoch(100);
        let ret = h
            .remove_expired_allocations(&rt, CLIENT1, vec![], alloc1.size.0)
            .unwrap();
        assert_eq!(vec![1], ret.considered);
        assert_eq!(vec![ExitCode::OK], ret.results.codes());
        assert_eq!(DataCap::from(alloc1.size.0), ret.datacap_recovered);
        ...
        rt.set_epoch(200);
        let ret = h
            .remove_expired_allocations(&rt, CLIENT1, vec![], alloc2.size.0)
            .unwrap();
        ...
        rt.replace_state(&state_with_allocs);
        let ret = h
            .remove_expired_allocations(&rt, CLIENT1, vec![], total_size)
            .unwrap();
        assert_eq!(vec![1, 2], ret.considered);
        assert_eq!(vec![ExitCode::OK, ExitCode::OK], ret.results.codes());
        assert_eq!(DataCap::from(total_size), ret.datacap_recovered);
    ...
    fn claim_allocs() {
        ...
        let prior_state: State = rt.get_state();
        {
            ...
            let ret = h
                .claim_allocations(&rt, PROVIDER1, reqs, size * 2, false)
                .unwrap();
        ...
        }
        {
            ...
            let ret = h
                .claim_allocations(&rt, PROVIDER1, reqs, size, false)
                .unwrap();
            assert_eq!(
                ret.batch_info.codes(),
                vec![ExitCode::OK, ExitCode::USR_NOT_FOUND]
            );
    ...
    fn get_claims() {
        ...
        let claim1 = make_claim(
            "1", CLIENT1, PROVIDER1, size, min_term, max_term, start, sector,
        );
        let claim2 = make_claim(
            "2", CLIENT1, PROVIDER1, size, min_term, max_term, start, sector,
        );
        let claim3 = make_claim(
            "3", CLIENT1, PROVIDER2, size, min_term, max_term, start, sector,
        );
    ...
    fn expire_claims() {
        ...
        let ret = h
            .remove_expired_claims(&rt, PROVIDER1, vec![id1, id2])
            .unwrap();
        assert_eq!(vec![1, 2], ret.considered);
        assert_eq!(
            vec![ExitCode::USR_FORBIDDEN, ExitCode::USR_FORBIDDEN],
...
mod datacap {
    ...
    use fil_actors_runtime_v11::cbor::serialize;
    use fil_actors_runtime_v11::runtime::policy_constants::{
    ...
    fn receive_tokens_make_allocs() {
...

        {
            ...
            h.receive_tokens(&rt, payload, BatchReturn::ok(2), BATCH_EMPTY, vec![1, 2], 0)
                .unwrap();
        ...
        }
        {
            ...
            h.receive_tokens(&rt, payload, BatchReturn::ok(1), BATCH_EMPTY, vec![3], 0)
                .unwrap();
    ...
    fn receive_tokens_extend_claims() {
        ...
        let claim1 = make_claim(
            "1", CLIENT1, PROVIDER1, SIZE, term_min, term_max, term_start, sector,
        );
        let claim2 = make_claim(
            "2",
            CLIENT2,
    ...
    fn receive_alloc_requires_miner_actor() {
        ...
        expect_abort_contains_message(
            ExitCode::USR_ILLEGAL_ARGUMENT,
            format!(
                "allocation provider {} must be a miner actor",
                provider1.id().unwrap()
            )
            .as_str(),
    ...
    fn receive_invalid_alloc_reqs() {
        ...
        // Tokens received doesn't match sum of allocation sizes
        {
            let reqs = vec![
                make_alloc_req(&rt, PROVIDER1, SIZE),
                make_alloc_req(&rt, PROVIDER2, SIZE),
            ];
    ...
    fn receive_invalid_extension_reqs() {
        ...
        // Extension too long
        {
            ...
            let payload = make_receiver_hook_token_payload(CLIENT1, vec![], reqs, SIZE);
            h.receive_tokens(&rt, payload, BATCH_EMPTY, BatchReturn::ok(1), vec![], SIZE)
                .unwrap();
            h.check_state(&rt);
        }
        {
