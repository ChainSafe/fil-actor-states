...
impl Harness {
    ...
    pub fn assert_verifier_removed(&self, rt: &MockRuntime, verifier: &Address) {
        ...
        let verifiers = rt.get_state::<State>().load_verifiers(&rt.store).unwrap();
        assert!(!verifiers
            .contains_key(&verifier_id_addr.to_bytes())
            .unwrap())
    }
    ...
    pub fn claim_allocations(
        ...
        if datacap_burnt > 0 {
            rt.expect_send_simple(
                ...
                .unwrap(),
                TokenAmount::zero(),
                IpldBlock::serialize_cbor(&BurnReturn {
    ...
    pub fn remove_expired_allocations(
        ...
        let params = RemoveExpiredAllocationsParams {
            client,
            allocation_ids,
        };
        let ret = rt
    ...
    pub fn remove_expired_claims(
        ...
        let params = RemoveExpiredClaimsParams {
            provider,
            claim_ids,
        };
        let ret = rt
