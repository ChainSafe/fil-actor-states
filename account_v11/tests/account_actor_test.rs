...
use fil_actors_runtime_v11::builtin::SYSTEM_ACTOR_ADDR;
...
fn test_fallback() {
    ...
    rt.expect_validate_caller_any();
    let result = rt
        .call::<AccountActor>(FIRST_EXPORTED_METHOD_NUMBER, params.clone())
        .unwrap();
    assert!(result.is_none());

    rt.expect_validate_caller_any();
    let result = rt
        .call::<AccountActor>(FIRST_EXPORTED_METHOD_NUMBER + 1, params.clone())
        .unwrap();
    assert!(result.is_none());
