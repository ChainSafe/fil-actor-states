...
use fil_actors_shared::actor_error_v19;
use fil_actors_shared::v19::runtime::Policy;
use fil_actors_shared::v19::{ActorDowncast, ActorError, Array, AsActorError};

use crate::v19::SECTORS_AMT_BITWIDTH;
...
impl Deadlines {
    ...
    pub fn load_deadline<BS: Blockstore>(
        ...
        store
            ...
            .ok_or_else(|| actor_error_v19!(illegal_argument, "no deadline {}", idx))
    }
...
impl Deadline {
    ...
    pub fn load_partition<BS: Blockstore>(
        ...
        let partition = partitions
            ...
            .ok_or_else(|| actor_error_v19!(not_found, "no partition {}", partition_idx))?;
        Ok(partition.clone())
    }
    ...
    pub fn load_partition_snapshot<BS: Blockstore>(
        ...
        let partition = partitions
            ...
            .ok_or_else(|| {
                actor_error_v19!(not_found, "no partition snapshot {}", partition_idx)
            })?;
    ...
    pub fn pop_expired_sectors<BS: Blockstore>(
        ...
        let new_early_terminations = BitField::try_from_bits(partitions_with_early_terminations)
            .map_err(
                |_| actor_error_v19!(illegal_state; "partition index out of bitfield range"),
            )?;
    ...
    pub fn terminate_sectors<BS: Blockstore>(
        ...
        for (partition_idx, sector_numbers) in partition_sectors.iter() {
            let mut partition = partitions
                ...
                .ok_or_else(
                    || actor_error_v19!(not_found; "failed to find partition {}", partition_idx),
                )?
                .clone();
    ...
    pub fn compact_partitions<BS: Blockstore>(
        ...
        let to_remove_set: BTreeSet<_> = to_remove
            ...
            .ok_or_else(
                || actor_error_v19!(illegal_argument; "partitions to remove exceeds total"),
            )?
            .collect();
        ...
        if let Some(&max_partition) = to_remove_set.iter().max() {
            if max_partition >= partition_count {
                return Err(
                    actor_error_v19!(illegal_argument; "partition index {} out of range [0, {})", max_partition, partition_count).into()
                );
            }
        } else {
            // Nothing to do.
        ...
        if !self.early_terminations.is_empty() {
            return Err(
                actor_error_v19!(illegal_argument; "cannot remove partitions from deadline with early terminations").into(),
            );
        }
    ...
    pub fn record_faults<BS: Blockstore>(
        ...
        for (partition_idx, sector_numbers) in partition_sectors.iter() {
            let mut partition = partitions
                ...
                .ok_or_else(|| actor_error_v19!(not_found; "no such partition {}", partition_idx))?
                .clone();
    ...
    pub fn process_deadline_end<BS: Blockstore>(
        ...
        for partition_idx in 0..partitions.count() {
            ...
            let mut partition = partitions
                ...
                .ok_or_else(|| actor_error_v19!(illegal_state; "no partition {}", partition_idx))?
                .clone();
...
impl Deadline {
    ...
    pub fn record_proven_sectors<BS: Blockstore>(
        ...
        let partition_indexes = BitField::try_from_bits(post_partitions.iter().map(|p| p.index))
            .map_err(
                |_| actor_error_v19!(illegal_argument; "partition index out of bitfield range"),
            )?;

        let num_partitions = partition_indexes.len();
        if num_partitions != post_partitions.len() as u64 {
            return Err(anyhow!(actor_error_v19!(
                illegal_argument,
        ...
        for post in post_partitions {
            let mut partition = partitions
                ...
                .ok_or_else(|| actor_error_v19!(not_found; "no such partition {}", post.index))?
                .clone();
    ...
    pub fn take_post_proofs<BS: Blockstore>(
        ...
        let post = proof_arr
            ...
            .ok_or_else(|| actor_error_v19!(illegal_argument, "proof {} not found", idx))?;
