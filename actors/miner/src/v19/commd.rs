...
use fil_actors_shared::v19::ActorError;
...
fn zero_commd(seal_proof: RegisteredSealProof) -> Result<Cid, ActorError> {
    ...
    let i = match seal_proof {
        ...
        _ => {
            return Err(actor_error_v19!(illegal_argument, "unknown SealProof"));
        }
    };
    let hash = Multihash::wrap(SHA2_256_TRUNC254_PADDED, &ZERO_COMMD_HASH[i])
        .map_err(|_| actor_error_v19!(assertion_failed, "static commd payload invalid"))?;
