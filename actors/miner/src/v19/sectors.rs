...
impl<'db, BS: Blockstore> Sectors<'db, BS> {
    ...
    pub fn load_sectors(
        ...
        for sector_number in sector_numbers.iter() {
            let sector_on_chain = self
                ...
                .cloned()
                .ok_or_else(
                    || actor_error_v19!(not_found; "sector not found: {}", sector_number),
                )?;
    ...
    pub fn delete_sectors(&mut self, sector_numbers: &BitField) -> Result<(), ActorError> {
        for sector_num in sector_numbers.iter() {
            ...
            if deleted_sector.is_none() {
                return Err(actor_error_v19!(not_found; "sector not found: {}", sector_num));
            }
        }
        Ok(())
    }
    ...
    pub fn must_get(&self, sector_number: SectorNumber) -> Result<SectorOnChainInfo, ActorError> {
        self.get(sector_number)?
            .ok_or_else(|| actor_error_v19!(not_found, "sector {} not found", sector_number))
    }
