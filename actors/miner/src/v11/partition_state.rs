...
use fil_actors_shared::v11::{ActorDowncast, Array};
use fvm_ipld_bitfield::BitField;
use fvm_ipld_blockstore::Blockstore;
use fvm_ipld_encoding::tuple::*;
use fvm_shared3::bigint::bigint_ser;
use fvm_shared3::clock::{ChainEpoch, NO_QUANTIZATION, QuantSpec};
use fvm_shared3::econ::TokenAmount;
use fvm_shared3::error::ExitCode;
use fvm_shared3::sector::{SectorSize, StoragePower};
use num_traits::{Signed, Zero};

use super::{
    BitFieldQueue, ExpirationQueue, ExpirationSet, SectorOnChainInfo, Sectors, TerminationResult,
    power_for_sectors, select_sectors, validate_partition_contains_sectors,
};
...
impl Partition {
    ...
    pub fn record_faults<BS: Blockstore>(
        ...
        validate_partition_contains_sectors(self, sector_numbers)
            .map_err(|e| actor_error_v11!(illegal_argument; "failed fault declaration: {}", e))?;
    ...
    pub fn terminate_sectors<BS: Blockstore>(
        ...
        if !live_sectors.contains_all(sector_numbers) {
            return Err(
                actor_error_v11!(illegal_argument, "can only terminate live sectors").into(),
            );
        }
