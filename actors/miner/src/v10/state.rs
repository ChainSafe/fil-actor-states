...
impl State {
    ...
    pub fn get_info<BS: Blockstore>(&self, store: &BS) -> anyhow::Result<MinerInfo> {
        match store.get_cbor(&self.info) {
            ...
            Ok(None) => Err(actor_error_v10!(not_found, "failed to get miner info").into()),
    ...
    pub fn allocate_sector_numbers<BS: Blockstore>(
        ...
        let prior_allocation = store
            ...
            .ok_or_else(|| {
                actor_error_v10!(illegal_state, "allocated sectors bitfield not found")
            })?;
    ...
    pub fn load_deadlines<BS: Blockstore>(&self, store: &BS) -> Result<Deadlines, ActorError> {
        store
            ...
            .ok_or_else(
                || actor_error_v10!(illegal_state; "failed to load deadlines {}", self.deadlines),
            )
    }
    ...
    pub fn load_vesting_funds<BS: Blockstore>(&self, store: &BS) -> anyhow::Result<VestingFunds> {
        Ok(store
            ...
            .ok_or_else(
                || actor_error_v10!(not_found; "failed to load vesting funds {:?}", self.vesting_funds),
            )?)
    }
    ...
    pub fn get_all_precommitted_sectors<BS: Blockstore>(
        ...
        for sector_no in sector_nos.iter() {
            if sector_no > MAX_SECTOR_NUMBER {
                return Err(
                    actor_error_v10!(illegal_argument; "sector number greater than maximum").into(),
                );
            }
            let info: &SectorPreCommitOnChainInfo = precommitted
                .get(&u64_key(sector_no))?
                .ok_or_else(|| actor_error_v10!(not_found, "sector {} not found", sector_no))?;
...
impl MinerInfo {
    pub fn new(
        ...
        let sector_size = window_post_proof_type
            .sector_size()
            .map_err(|e| actor_error_v10!(illegal_argument, "invalid sector size: {}", e))?;

        let window_post_partition_sectors = window_post_proof_type
            .window_post_partitions_sector()
            .map_err(|e| actor_error_v10!(illegal_argument, "invalid partition sectors: {}", e))?;
