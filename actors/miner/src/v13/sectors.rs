...
use fvm_shared4::sector::{MAX_SECTOR_NUMBER, SectorNumber};
...
impl<'db, BS: Blockstore> Sectors<'db, BS> {
    ...
    pub fn load_sector(
        ...
        for sector_number in sector_numbers.iter() {
            let sector_on_chain = self
                ...
                .cloned()
                .ok_or_else(
                    || actor_error_v13!(not_found; "sector not found: {}", sector_number),
                )?;
    ...
    pub fn must_get(&self, sector_number: SectorNumber) -> Result<SectorOnChainInfo, ActorError> {
        self.get(sector_number)?
            .ok_or_else(|| actor_error_v13!(not_found, "sector {} not found", sector_number))
    }
