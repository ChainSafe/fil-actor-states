...
use fil_actors_shared::v12::{ActorDowncast, ActorError, Array, AsActorError};
...
use crate::v12::SECTORS_AMT_BITWIDTH;
...
impl Deadlines {
    ...
    pub fn load_deadline<BS: Blockstore>(
        ...
        store
            ...
            .ok_or_else(|| actor_error_v12!(illegal_argument, "no deadline {}", idx))
    }
    ...
    pub fn move_partitions<BS: Blockstore>(
        ...
        for (i, orig_partition_idx) in partitions.iter().enumerate() {
            let moving_partition = orig_partitions
                ...
                .ok_or_else(|| actor_error_v12!(not_found, "no partition {}", orig_partition_idx))?
                .clone();
            if !moving_partition.faults.is_empty() || !moving_partition.unproven.is_empty() {
                return Err(actor_error_v12!(forbidden, "partition with faults or unproven sectors are not allowed to move, partition_idx {}", orig_partition_idx))?;
            }
            if orig_deadline.early_terminations.get(orig_partition_idx) {
                return Err(actor_error_v12!(forbidden, "partition with early terminated sectors are not allowed to move, partition_idx {}", orig_partition_idx))?;
            }
        ...
        // we explicitly keep the `dest_epoch` the same as `orig_epoch`, this is by design of not re-quantizing.
        {
            ...
            orig_expirations_epochs.for_each_mut(|orig_epoch, orig_bitfield| {
                ...
                let mut to_bitfield = dest_expirations_epochs
                    .get(dest_epoch)?
                    .cloned()
                    .unwrap_or_default();
...
impl Deadline {
    ...
    pub fn load_partition<BS: Blockstore>(
        ...
        let partition = partitions
            ...
            .ok_or_else(|| actor_error_v12!(not_found, "no partition {}", partition_idx))?;
    ...
    pub fn load_partition_snapshot<BS: Blockstore>(
        ...
        let partition = partitions
            ...
            .ok_or_else(|| {
                actor_error_v12!(not_found, "no partition snapshot {}", partition_idx)
            })?;
    ...
    pub fn pop_expired_sectors<BS: Blockstore>(
        ...
        let new_early_terminations = BitField::try_from_bits(partitions_with_early_terminations)
            .map_err(
                |_| actor_error_v12!(illegal_state; "partition index out of bitfield range"),
            )?;
    ...
    pub fn terminate_sectors<BS: Blockstore>(
        ...
        for (partition_idx, sector_numbers) in partition_sectors.iter() {
            let mut partition = partitions
                ...
                .ok_or_else(
                    || actor_error_v12!(not_found; "failed to find partition {}", partition_idx),
                )?
                .clone();
    ...
    > {
        ...
        let to_remove_set: BTreeSet<_> = to_remove
            ...
            .ok_or_else(
                || actor_error_v12!(illegal_argument; "partitions to remove exceeds total"),
            )?
            .collect();
        ...
        if let Some(&max_partition) = to_remove_set.iter().max() {
            if max_partition >= partition_count {
                return Err(
                    actor_error_v12!(illegal_argument; "partition index {} out of range [0, {})", max_partition, partition_count).into()
                );
            }
        } else {
            // Nothing to do.
        ...
        if !self.early_terminations.is_empty() {
            return Err(
                actor_error_v12!(illegal_argument; "cannot remove partitions from deadline with early terminations").into(),
            );
        }
    ...
    pub fn record_faults<BS: Blockstore>(
        ...
        for (partition_idx, sector_numbers) in partition_sectors.iter() {
            let mut partition = partitions
                ...
                .ok_or_else(|| actor_error_v12!(not_found; "no such partition {}", partition_idx))?
                .clone();
    ...
    pub fn process_deadline_end<BS: Blockstore>(
        ...
        for partition_idx in 0..partitions.count() {
            ...
            let mut partition = partitions
                ...
                .ok_or_else(|| actor_error_v12!(illegal_state; "no partition {}", partition_idx))?
                .clone();
...
impl Deadline {
    ...
    pub fn record_proven_sectors<BS: Blockstore>(
        ...
        let partition_indexes = BitField::try_from_bits(post_partitions.iter().map(|p| p.index))
            .map_err(
                |_| actor_error_v12!(illegal_argument; "partition index out of bitfield range"),
            )?;

        let num_partitions = partition_indexes.len();
        if num_partitions != post_partitions.len() as u64 {
            return Err(anyhow!(actor_error_v12!(
                illegal_argument,
        ...
        for post in post_partitions {
            let mut partition = partitions
                ...
                .ok_or_else(|| actor_error_v12!(not_found; "no such partition {}", post.index))?
                .clone();
    ...
    pub fn take_post_proofs<BS: Blockstore>(
        ...
        let post = proof_arr
            ...
            .ok_or_else(|| actor_error_v12!(illegal_argument, "proof {} not found", idx))?;
