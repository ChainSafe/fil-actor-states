...
use fil_actors_shared::v14::{ActorDowncast, Array};
...
impl Partition {
    ...
    pub fn record_faults<BS: Blockstore>(
        ...
        validate_partition_contains_sectors(self, sector_numbers)
            .map_err(|e| actor_error_v14!(illegal_argument; "failed fault declaration: {}", e))?;
    ...
    pub fn terminate_sectors<BS: Blockstore>(
        ...
        if !live_sectors.contains_all(sector_numbers) {
            return Err(actor_error_v14!(illegal_argument, "can only terminate live sectors").into());
        }
