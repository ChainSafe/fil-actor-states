...
use fil_actors_shared::v18::{ActorError, AsActorError, Config, DEFAULT_HAMT_CONFIG, Map2, MapMap};

use crate::v18::{AddrPairKey, AllocationID, ClaimID};
use crate::v18::{DataCap, RemoveDataCapProposalID};
    ...
    pub fn new<BS: Blockstore>(store: &BS, root_key: Address) -> Result<State, ActorError> {
        ...
        let empty_allocs_claims =
            MapMap::<_, (), ActorID, u64>::new(store, HAMT_BIT_WIDTH, HAMT_BIT_WIDTH)
                .flush()
                .map_err(|e| {
                    actor_error_v18!(illegal_state, "failed to create empty multi map: {}", e)
                })?;
