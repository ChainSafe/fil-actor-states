...
use fil_actors_shared::v16::{ActorError, AsActorError, Config, Map2, MapMap, DEFAULT_HAMT_CONFIG};

use crate::v16::{AddrPairKey, AllocationID, ClaimID};
use crate::v16::{DataCap, RemoveDataCapProposalID};
    ...
    pub fn new<BS: Blockstore>(store: &BS, root_key: Address) -> Result<State, ActorError> {
        ...
        let empty_allocs_claims =
            MapMap::<_, (), ActorID, u64>::new(store, HAMT_BIT_WIDTH, HAMT_BIT_WIDTH)
                .flush()
                .map_err(|e| {
                    actor_error_v16!(illegal_state, "failed to create empty multi map: {}", e)
                })?;
