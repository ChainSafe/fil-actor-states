...
impl State {
    pub fn new<BS: Blockstore>(store: &BS) -> Result<Self, ActorError> {
        ...
        let empty_deal_ops_hamt = SetMultimap::new(store).root().context_code(
            ExitCode::USR_ILLEGAL_STATE,
            "Failed to create empty multiset",
        )?;
    ...
    where
        BS: Blockstore,
    {
        let states = DealMetaArray::load(&self.states, store).context_code(
            ExitCode::USR_ILLEGAL_STATE,
    ...
    where
        BS: Blockstore,
    {
        let mut states = DealMetaArray::load(&self.states, store).context_code(
            ExitCode::USR_ILLEGAL_STATE,
    ...
    where
        BS: Blockstore,
    {
        let deal_proposals = DealArray::load(&self.proposals, store).context_code(
            ExitCode::USR_ILLEGAL_STATE,
            "failed to load deal proposal array",
        )?;
        ...
        BS: Blockstore,
    {
        ...
        self.deal_ops_by_epoch = deals_by_epoch.root().context_code(
            ExitCode::USR_ILLEGAL_STATE,
            "failed to flush deals by epoch",
        )?;
        ...
        BS: Blockstore,
    {
        ...
        if &prev_locked + amount > escrow_balance {
            return Err(actor_error_v10!(insufficient_funds;
        ...
        BS: Blockstore,
    {
        ...
        locked_table.must_subtract(addr, amount).context_code(
            ExitCode::USR_ILLEGAL_STATE,
            "subtract from locked table failed",
        )?;
