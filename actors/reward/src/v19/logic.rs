...
use fil_actors_shared::v19::builtin::reward::math::PRECISION;
...
mod tests {
    ...
    fn test_baseline_reward() {
        ...
        let filename = "src/v19/testdata/TestBaselineReward.golden";
        let golden_contents =
    ...
    fn test_canonical_baseline_reward_samples() {
        ...
        let samples = [
            (
                0,
                "0",
                "340282366920938463463374607398434878123",
                "84438390138547108892",
            ),
            (
                1,
        ...
        for (index, prev_theta, curr_theta, expected_atto) in samples {
            ...
            assert_eq!(
                expected_atto,
                reward.atto().to_string(),
                "baseline reward sample {index}"
            );
        }
    }
    ...
    fn test_simple_reward() {
        ...
        let filename = "src/v19/testdata/TestSimpleReward.golden";
        let golden_contents =
            fs::read_to_string(filename).expect("Something went wrong reading the file");
