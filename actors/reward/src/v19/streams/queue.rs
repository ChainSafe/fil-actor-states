...
impl QueuedCall {
    ...
    fn encode(&self) -> Result<RawBytes> {
        Ok(match self {
            QueuedCall::Weights { updates, .. } => RawBytes::serialize(&WeightRecordsPayload {
...
impl Ledger {
    ...
    fn apply(&mut self, call: &QueuedCall, effective: ChainEpoch) -> Result<TokenAmount, Stranded> {
        let dust = match call {
            ...
            QueuedCall::Register {
                ...
                self.streams.insert_stream(Stream {
                    id,
                    weight: weight.clone(),
                    distribution,
                });
    ...
    pub(crate) fn admit(
        ...
        debug_assert!(
            self.streams
                .pending_writes
                .iter()
                .all(|write| write.effective_epoch >= epoch),
        ...
        let (baseline, before) = match self.project(epoch, Baseline::Valid) {
            ...
            Err(_) if slot == Slot::ScheduleWide(PendingWriteOp::SetWeightRecords) => (
                Baseline::Repairing,
                self.project(epoch, Baseline::Repairing)?,
            ),
        ...
        self.streams
            .pending_writes
            .push(call.queue_entry(effective)?);
        // Stable sorting preserves insertion order among calls effective at the same epoch.
        self.streams
            .pending_writes
            .sort_by_key(|write| write.effective_epoch);
        self.streams_dirty = true;

        let after = self.project(epoch, baseline)?;
        if let Some((_, stranded)) = after
            .stranded
            .iter()
            .find(|(occupied, _)| *occupied == slot)
        {
            return Err(anyhow::anyhow!(
                "pending call {slot:?} is invalid: {stranded}"
            ));
        }
        ensure!(
    ...
    pub(crate) fn cancel(&mut self, slot: Slot) -> Option<PendingWrite> {
        ...
        let idx = self
            ...
            .pending_writes
            .iter()
            .position(|write| write.slot() == slot)?;
