...
//!
//! The shapes it reads and writes are in [`crate::v19::state`]: [`ExplicitDistribution`], its
...
impl Ledger {
    ...
    pub(super) fn remove_stream(&mut self, id: StreamId) -> Result<TokenAmount, Stranded> {
        let mut stream = self
            .streams
            .take_stream(id)
            .ok_or(Stranded::MissingStream(id))?;
        ...
        if !distribution.payable.is_empty() {
            self.streams
                .insert_tombstone(id, std::mem::take(&mut distribution.payable));
        }
        Ok(burn)
    }
