...
// The callers of most items below live in upstream lib.rs, which fil-actor-states does not carry.
#![allow(dead_code, unused_imports)]

use anyhow::Result;
use fil_actors_shared::actor_error_v19;
use fil_actors_shared::v19::runtime::Runtime;
use fil_actors_shared::v19::{ActorDowncast, ActorError};
...
use crate::v19::state::{
    ExplicitDistribution, State, Stream, StreamAccrual, StreamId, StreamsState,
};
...
impl Ledger {
    ...
    pub(crate) fn load(rt: &impl Runtime, st: &State) -> Result<Ledger, ActorError> {
        ...
        Ledger::checked(streams, st.accrued.clone()).map_err(|e| {
            e.downcast_default(
                ...
                "invalid persisted stream state",
            )
        })
    }
    ...
    pub(crate) fn store(&self, rt: &impl Runtime, st: &mut State) -> Result<(), ActorError> {
        ...
        if self.streams_dirty {
            st.streams_root = rt
                .store()
                .put_cbor(&self.streams, Code::Blake2b256)
                .map_err(|e| {
...
impl Ledger {
    ...
    pub(crate) fn corrupt(&mut self) {
        self.accrued.push(StreamAccrual {
            id: StreamId::MAX,
            amount: TokenAmount::zero(),
        });
    }
