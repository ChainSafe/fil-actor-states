...
fn shares(rows: &[(u64, u64)]) -> Vec<RecipientShare> {
    rows.iter()
        .map(|(id, share)| RecipientShare {
            recipient: Address::new_id(*id),
            share: *share,
        })
        .collect()
}
...
fn next_epoch(streams: &StreamsState) -> ChainEpoch {
    streams
        .pending_writes
        .first()
        .map_or(EPOCH_UNDEFINED, |write| write.effective_epoch)
}
...
fn deferred_payloads_have_stable_cbor() {
    ...
    assert_eq!(
        ...
        fvm_ipld_encoding::RawBytes::serialize(&weight)
            .unwrap()
            .bytes()
    );
    assert_eq!(
        hex!("818282018502200300048205850800070809").as_slice(),
        fvm_ipld_encoding::RawBytes::serialize(&batch)
            .unwrap()
            .bytes()
    );
    assert_eq!(
    ...
    assert_eq!(
        hex!("80").as_slice(),
        fvm_ipld_encoding::RawBytes::new(vec![0x80]).bytes()
    );
    assert_eq!(
...
fn removal_admission_reserves_tombstone_rows_and_claim_relieves_the_bound() {
    ...
    assert!(
        error.to_string().contains("tombstone row reservation 257"),
        "{error}"
    );
...
fn pending_removal_bound_survives_share_folds_during_the_hold() {
    ...
    let replacement: Vec<_> = (0..MAX_RECIPIENTS)
        .map(|offset| RecipientShare {
            recipient: Address::new_id(2_000 + offset as u64),
            share,
        })
        .collect();
...
impl SupplyTracker {
    fn award(
        ...
        assert!(
            allocation.schedule_valid,
            "valid randomized state entered degradation"
        );
    ...
    fn pay_claim(&mut self, result: &[TokenAmount]) {
        let paid = result
            .iter()
            .fold(TokenAmount::zero(), |total, amount| total + amount);
...
fn allocates_reward_in_stream_order_and_conserves_attos() {
    ...
    let mut accruals = vec![
        StreamAccrual {
            id: 2,
            amount: TokenAmount::from_atto(4),
        },
        StreamAccrual {
            id: 3,
...
fn burns_sentinel_share_without_accruing_or_tombstoning_it() {
    ...
    let mut accruals = vec![StreamAccrual {
        id: 2,
        amount: TokenAmount::zero(),
    }];
    ...
    assert_eq!(
        reward,
        &partial.miner + &partial.service[0].amount + &partial.burn
    );
    ...
    let claimed = claim(
        ...
        &accruals,
        2,
        &[Address::new_id(101), BURNT_FUNDS_ACTOR_ADDR],
    )
    .unwrap();
    assert_eq!(
        vec![TokenAmount::from_atto(5), TokenAmount::zero()],
        claimed
    );
    let distribution = streams.streams[1].distribution.as_ref().unwrap();
    ...
    assert_eq!(
        reward,
        &removed.miner + &removed.service[0].amount + &removed.burn
    );
    ...
    let claimed = claim(
        ...
        &accruals,
        2,
        &[BURNT_FUNDS_ACTOR_ADDR, Address::new_id(102)],
    )
    .unwrap();
    assert_eq!(
        vec![TokenAmount::zero(), TokenAmount::from_atto(5)],
        claimed
    );
...
fn indivisible_sentinel_portion_preserves_survivor_entitlements() {
    ...
    let mut ordinary = StreamsState {
        streams: vec![stream(
            ...
            Some(explicit(
                200,
                shares(&[(101, third), (102, third), (103, DENOM - 2 * third)]),
            )),
        )],
        ..Default::default()
    };
    ...
    let ordinary_claims = claim(
        &mut ordinary,
        &ordinary_accruals,
        2,
        &[Address::new_id(101), Address::new_id(102)],
    )
    .unwrap();
    assert_eq!(
        vec![TokenAmount::zero(), TokenAmount::zero()],
        ordinary_claims
    );
    ...
    assert_eq!(
        ...
        sentinel_allocation.service[0].amount
    );
    ...
    accrue_service(
        ...
        &allocate_reward(&ordinary.streams, 0, &reward)
            .unwrap()
            .service,
    )
    .unwrap();
    accrue_service(
        ...
        &allocate_reward(&sentinel.streams, 0, &reward)
            .unwrap()
            .service,
    )
    .unwrap();
    ...
    .unwrap();
    assert_eq!(
        vec![TokenAmount::from_atto(1), TokenAmount::from_atto(1)],
        ordinary_claims
    );
    assert_eq!(ordinary_claims, sentinel_claims);
}
...
fn admits_and_strips_burn_sentinel_rows() {
    ...
    over_limit.extend((0..MAX_RECIPIENTS).map(|_| RecipientShare {
        recipient: BURNT_FUNDS_ACTOR_ADDR,
        share: 1,
    }));
...
fn folds_period_under_outgoing_shares_and_burns_only_dust() {
    ...
    assert_eq!(
        ...
        amount(&distribution.payable, 101)
    );
    assert_eq!(
        TokenAmount::from_atto(3),
        amount(&distribution.payable, 102)
    );
    assert_eq!(
        TokenAmount::from_atto(3),
        amount(&distribution.payable, 103)
    );
    assert_eq!(
        TokenAmount::from_atto(5),
        amount(&distribution.payable, 104)
    );
}
...
fn caps_payable_rows_at_128_rejects_129_atomically_and_recovers_through_claims() {
    ...
    let mut accruals = vec![StreamAccrual {
        id: 2,
        amount: TokenAmount::from_atto(MAX_RECIPIENTS as u64),
    }];
    ...
    assert_eq!(
        MAX_PAYABLE_ROWS_PER_STREAM,
        removed_streams.tombstones[0].payable.len()
    );
    ...
    assert!(
        first_batch
            .iter()
            .all(|amount| amount == &TokenAmount::from_atto(2))
    );
    assert_eq!(
        MAX_RECIPIENTS,
        streams.streams[0]
    ...
    assert!(
        second_batch
            .iter()
            .all(|amount| amount == &TokenAmount::from_atto(1))
    );
    assert!(
        streams.streams[0]
            .distribution
...
fn structural_validation_rejects_unordered_stored_shares() {
    ...
    let streams = StreamsState {
        streams: vec![stream(2, pct(20), Some(unordered))],
        ..Default::default()
    };
    ...
    assert!(
        error
            .to_string()
            .contains("stored share recipients are not ordered"),
        "{error}"
    );
}
...
fn claims_live_and_payable_amounts_once_in_request_order() {
    ...
    let accruals = vec![StreamAccrual {
        id: 2,
        amount: TokenAmount::from_atto(11),
    }];
    let wallets = [
        Address::new_id(101),
        Address::new_id(101),
    ...
    assert_eq!(
        TokenAmount::from_atto(5),
        amount(&distribution.claimed_period, 101)
    );
    assert_eq!(
        TokenAmount::from_atto(5),
        amount(&distribution.claimed_period, 102)
    );
    let before = streams.clone();
...
fn claims_tombstones_and_deletes_them_when_drained() {
    ...
    let result = claim(
        ...
        &[],
        3,
        &[Address::new_id(102), Address::new_id(999)],
    )
    .unwrap();
...
fn rejects_a_new_call_that_strands_an_existing_call() {
    ...
    assert!(
        error
            .to_string()
            .contains("invalidates an existing pending call")
    );
...
fn rejects_single_epoch_overlap_on_either_side_of_a_clamp_crossing() {
    ...
    for case in cases {
        assert_eq!(
            1,
            (case.violating_epoch - case.crossing_epoch).abs(),
            "{}",
            case.name
        );
        ...
        assert!(
            weight_sum(case.violating_epoch - 1) <= u128::from(DENOM),
            "{}",
            case.name
        );
        assert!(
            weight_sum(case.violating_epoch) > u128::from(DENOM),
            "{}",
            case.name
        );
        assert!(
        ...
        assert!(
            error
                .to_string()
                .contains(&format!("exceed DENOM at epoch {}", case.violating_epoch)),
            "{}: {error}",
            case.name
        );
...
fn queues_batches_cancels_slots_and_tracks_queue_head() {
    ...
    let updates = [
        WeightRecordUpdate {
            id: 1,
            weight: constant_weight(pct(65)),
        },
        WeightRecordUpdate {
            id: 2,
            weight: constant_weight(pct(35)),
        },
    ];
    ...
    assert_eq!(17, next_epoch(&streams));
    assert!(
        cancel_pending(&mut streams, None, PendingWriteOp::SetWeightRecords)
            .unwrap()
            .is_some()
    );
    assert_eq!(EPOCH_UNDEFINED, next_epoch(&streams));
...
fn apply_and_cancel_surfaces_calls_dropped_before_cancellation() {
    ...
    .unwrap();
    assert!(
        cancel_pending(&mut streams, Some(3), PendingWriteOp::RegisterStream)
            .unwrap()
            .is_some()
    );
    ...
    assert_eq!(
        PendingWriteOp::SetWeightRecords,
        result.apply_result.dropped[0].op
    );
    assert!(result.removed.is_none());
...
fn enforces_registration_bounds_and_id_availability() {
    ...
    let new_stream = stream(
        ...
        Some(explicit(
            201,
            shares(&[(103, DENOM / 2), (102, DENOM - DENOM / 2)]),
        )),
    );
    ...
    assert_eq!(
        17,
        queue_register_stream(&mut streams, 10, 7, new_stream, 17).unwrap()
    );
...
fn applies_same_epoch_lifecycle_writes_in_operation_order() {
    ...
    assert_eq!(
        vec![2],
        accruals.iter().map(|row| row.id).collect::<Vec<_>>()
    );
    assert_eq!(EPOCH_UNDEFINED, next_epoch(&streams));
}
...
fn drops_a_weight_batch_stranded_by_cancelled_registration() {
    ...
    assert_eq!(
        Address::new_id(999),
        streams.streams[1].distribution.as_ref().unwrap().writer
    );
    assert_eq!(EPOCH_UNDEFINED, next_epoch(&streams));
...
fn null_round_at_effective_epoch_cannot_revive_a_stranded_call() {
    ...
    queue_weight_records(
        ...
        &[WeightRecordUpdate {
            id: 2,
            weight: constant_weight(pct(45)),
        }],
    )
    .unwrap();
    assert!(
        cancel_pending(&mut streams, Some(1), PendingWriteOp::RemoveStream)
            .unwrap()
            .is_some()
    );
...
fn removal_settles_into_a_claimable_tombstone() {
    ...
    assert_eq!(
        TokenAmount::from_atto(3),
        amount(&streams.tombstones[0].payable, 101)
    );
    assert_eq!(
        TokenAmount::from_atto(8),
        amount(&streams.tombstones[0].payable, 102)
    );
    ...
    .unwrap();
    assert_eq!(
        vec![TokenAmount::from_atto(3), TokenAmount::from_atto(8)],
        claimed
    );
    assert!(streams.tombstones.is_empty());
}
...
fn conserves_service_value_across_claims_folds_and_removal() {
    ...
    assert_service_conserved(&gross, &paid, &burned, &streams, &accruals);
    queue_remove_stream(&mut streams, 0, 1, 2).unwrap();
    burned += apply_due_writes(&mut streams, &mut accruals, 1)
        .unwrap()
        .burn;
    assert_service_conserved(&gross, &paid, &burned, &streams, &accruals);
    ...
    let result = claim(
        ...
        &accruals,
        2,
        &[Address::new_id(102), Address::new_id(103)],
    )
    .unwrap();
    paid += result
        .iter()
...
fn randomized_conservation_covers_the_full_operation_mix_and_drops() {
    ...
    for _ in 0..512 {
        ...
        let previous = (
            supply.total_minted.clone(),
            supply.total_burn.clone(),
            supply.total_service.clone(),
        );
        match random_u64(&mut random) % covered.len() as u64 {
            ...
            1 => {
                    ...
                    if let Some(distribution) = &stream.distribution {
                        targets.extend(
                            distribution
                                .shares
                                .iter()
                                .map(|share| (stream.id, share.recipient)),
                        );
                        targets.extend(
                            distribution
                                .payable
                                .iter()
                                .map(|row| (stream.id, row.recipient)),
                        );
                    }
                }
                for tombstone in &streams.tombstones {
                    targets.extend(
                        tombstone
                            .payable
                            .iter()
                            .map(|row| (tombstone.id, row.recipient)),
                    );
                }
                let (id, wallet) =
                ...
                let result = claim(
                    ...
                    &accruals,
                    id,
                    &[wallet, wallet, Address::new_id(999_998)],
                )
                .unwrap();
            ...
            6 | 7 => {
                ...
                if queue_weight_records(&mut streams, epoch, 2, op, &[update]).is_ok() {
                    covered[if op == PendingWriteOp::SetWeightRecords {
                        6
                    } else {
                        7
                    }] = true;
    ...
    assert!(
        ...
        "missing operation coverage: {covered:?}"
    );
    assert!(dropped >= 2);
}
