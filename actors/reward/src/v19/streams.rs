...
use fil_actors_shared::v19::BURNT_FUNDS_ACTOR_ADDR;
...
pub fn compute_weight(record: &WeightRecord, epoch: ChainEpoch) -> u64 {
    ...
    u64::try_from(
        value
            .min(i128::from(record.cap))
            .max(i128::from(record.floor)),
    )
...
pub fn validate_weight_record(record: &WeightRecord) -> Result<()> {
        ...
        record.v_start >= record.floor,
        "weight v_start is below floor"
    );
...
fn validate_weight_updates(updates: &[WeightRecordUpdate]) -> Result<()> {
    ...
    for pair in updates.windows(2) {
            ...
            pair[0].id != pair[1].id,
            "duplicate weight-record stream ID {}",
            pair[0].id
        );
        ensure!(
            pair[0].id <= pair[1].id,
            "weight-record updates are not ordered"
        );
    }
...
pub fn weight_breakpoints(record: &WeightRecord, start_epoch: ChainEpoch) -> Vec<ChainEpoch> {
    ...
    if record.slope != 0 {
        ...
        insert_crossing(if record.slope > 0 {
            record.cap
        } else {
            record.floor
        });
...
fn validate_weight_schedule_through(
    ...
    end_epoch: ChainEpoch,
        ...
        "weight schedule interval is reversed"
    );
    ...
    for epoch in epochs {
        let sum: u128 = streams
            .iter()
            .map(|stream| u128::from(compute_weight(&stream.weight, epoch)))
            .sum();
        ensure!(
            sum <= u128::from(DENOM),
            "stream weights exceed DENOM at epoch {epoch}: {sum}"
        );
    }
    Ok(())
}
    ...
    for stream in streams {
        ...
        if let Some(distribution) = &stream.distribution {
            let share_total = stored_share_total(&distribution.shares)?;
                ...
                share_total <= DENOM,
                "stored shares sum to {share_total}, exceeds {DENOM}"
            );
...
fn validate_share_rows(shares: &[RecipientShare], form: ShareForm) -> Result<u128> {
    ...
    for row in shares {
        validate_id_address(&row.recipient, "share recipient")?;
            ...
            row.share != 0,
            "share for recipient {} is zero",
            row.recipient
        );
        if row.recipient == BURNT_FUNDS_ACTOR_ADDR {
            ensure!(
                form == ShareForm::Wire,
                "burn sentinel persisted as a recipient"
            );
        } else {
            ensure!(
...
pub fn validate_shares(shares: &[RecipientShare]) -> Result<()> {
    let total = validate_share_rows(shares, ShareForm::Wire)?;
        ...
        total == u128::from(DENOM),
        "shares sum to {total}, expected {DENOM}"
    );
    Ok(())
}
...
fn validate_stored_shares(shares: &[RecipientShare]) -> Result<u64> {
    let total = validate_share_rows(shares, ShareForm::Stored)?;
        ...
        total <= u128::from(DENOM),
        "stored shares sum to {total}, exceeds {DENOM}"
    );
    Ok(total as u64)
}
...
fn stored_share_total(shares: &[RecipientShare]) -> Result<u64> {
    shares.iter().try_fold(0_u64, |total, row| {
        total
            .checked_add(row.share)
            .ok_or_else(|| anyhow::anyhow!("stored shares overflow"))
    })
}
...
pub fn accrue_service(accruals: &mut [StreamAccrual], portions: &[StreamAccrual]) -> Result<()> {
    ...
    for portion in portions {
            ...
            seen.insert(portion.id),
            "duplicate service portion for stream {}",
            portion.id
        );
        ensure!(
...
pub fn compute_service_liability(
    ...
    for stream in &streams.streams {
        ...
        let claimed: TokenAmount = distribution
            .claimed_period
            .iter()
            .map(|row| &row.amount)
            .sum();
...
pub fn claim(
    ...
    } else if let Some(tombstone_idx) = streams
        .tombstones
        .iter()
...
fn validate_period_claims(distribution: &ExplicitDistribution, pool: &TokenAmount) -> Result<u64> {
    ...
    let share_total = validate_stored_shares(&distribution.shares)?;
        ...
        share_total != 0 || pool.is_zero(),
        "zero-share distribution has non-zero accrual"
    );
    let denom = BigInt::from(share_total);
...
fn validate_amount_rows(rows: &[RecipientAmount], label: &str) -> Result<()> {
    ...
    for row in rows {
        validate_id_address(&row.recipient, label)?;
            ...
            row.amount > TokenAmount::zero(),
            "{label} amount is not positive"
        );
    }
    Ok(())
}
...
pub fn queue_register_stream(
    ...
    {
        return Err(anyhow::anyhow!(
            "new stream has pre-existing accounting state"
        ));
    }
...
pub(crate) fn validate_cancel_target(id: Option<StreamId>, op: PendingWriteOp) -> Result<()> {
        ...
        op != PendingWriteOp::StepWeightRecords,
        "StepWeightRecords cannot be cancelled"
    );
...
pub fn apply_due_writes_and_cancel(
    ...
    *accruals = projected.accruals;
    Ok(CancelResult {
        apply_result: projected.apply_result,
        removed,
    })
}
...
pub fn apply_due_writes(
    ...
    if streams
        .pending_writes
        .first()
        .is_none_or(|write| write.effective_epoch > epoch)
    {
...
fn ensure_slot_available(
    ...
    let slot = pending_slot(id, op);
        ...
        !streams
            .pending_writes
            .iter()
            .any(|write| pending_slot(write.id, write.op) == slot),
...
fn validate_stream_configuration_without_weights(streams: &[Stream]) -> Result<()> {
        ...
        "stream count exceeds maximum {MAX_STREAMS}"
    );
    ensure!(
        streams.is_sorted_by(|a, b| a.id < b.id),
        "stream IDs are not ordered"
    );
    ensure!(
        !streams.iter().any(|stream| stream.id == 0),
        "stream ID 0 is reserved"
    );
    ensure!(
        streams
            .iter()
            .filter(|stream| stream.distribution.is_none())
            .count()
            <= 1,
...
fn validate_streams_state_structure_without_weights(
    ...
    validate_award_state_structure(streams)?;
        ...
        accruals.is_sorted_by(|a, b| a.id < b.id),
        "explicit-stream accruals are not ordered"
    );
...
pub(crate) fn validate_award_state_structure(streams: &StreamsState) -> Result<()> {
    ...
    validate_stream_configuration_without_weights(&streams.streams)?;
        ...
        "tombstones are not ordered"
    );

    let live_ids: BTreeSet<_> = streams.streams.iter().map(|stream| stream.id).collect();
    let tombstone_ids: BTreeSet<_> = streams
        .tombstones
        .iter()
        .map(|tombstone| tombstone.id)
        .collect();
    ensure!(
        live_ids.is_disjoint(&tombstone_ids),
        "a stream ID is live and tombstoned"
    );
    for tombstone in &streams.tombstones {
        ensure!(tombstone.id != 0, "stream ID 0 is reserved");
        ensure!(
            !tombstone.payable.is_empty(),
            "tombstone {} is empty",
            tombstone.id
        );
        validate_amount_rows(&tombstone.payable, "tombstone payable")?;
    ...
    for write in &streams.pending_writes {
        if write.op == PendingWriteOp::RegisterStream {
            ...
            let id = write
                .id
                .expect("validated per-stream pending call has an ID");
            ensure!(
...
fn validate_id_address(address: &Address, label: &str) -> Result<()> {
        ...
        "{label} {address} is not an ID address"
    );
    Ok(())
}

fn validate_tombstone_capacity(streams: &StreamsState) -> Result<()> {
    let mut rows: usize = streams
        .tombstones
        .iter()
    ...
    for write in &streams.pending_writes {
        ...
        rows += streams
            ...
            .map_or(MAX_RECIPIENTS, |distribution| {
                MAX_RECIPIENTS.max(recipient_union_len(
...
fn recipient_union_len(payable: &[RecipientAmount], shares: &[RecipientShare]) -> usize {
    ...
    while payable_idx < payable.len() && shares_idx < shares.len() {
        count += 1;
        match payable[payable_idx]
            .recipient
            .cmp(&shares[shares_idx].recipient)
        {
...
fn apply_pending_transition(
    ...
    match write.op {
        ...
        PendingWriteOp::RegisterStream => {
            let id = write
                .id
                .ok_or_else(|| anyhow::anyhow!("RegisterStream call has no stream ID"))?;
            ...
            let distribution = payload
                .distribution
                .map(|distribution| ExplicitDistribution {
                    writer: distribution.writer,
                    shares: distribution.shares,
                    payable: Vec::new(),
        ...
        PendingWriteOp::RemoveStream => {
            let id = write
                .id
                .ok_or_else(|| anyhow::anyhow!("RemoveStream call has no stream ID"))?;
            match accruals {
                Some(accruals) => {
        ...
        PendingWriteOp::SetDistribution => {
            let id = write
                .id
                .ok_or_else(|| anyhow::anyhow!("SetDistribution call has no stream ID"))?;
            let payload: SetDistributionPayload = write.payload.deserialize()?;
            match accruals {
...
fn remove_stream(
    ...
    if !distribution.payable.is_empty() {
        ...
        tombstones.push(Tombstone {
            id,
            payable: std::mem::take(&mut distribution.payable),
        });
