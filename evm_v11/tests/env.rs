...
impl TestEnv {
    ...
    pub fn new(evm_address: Address) -> Self {
        ...
        runtime
            .actor_code_cids
            .borrow_mut()
            .insert(evm_address, *EVM_ACTOR_CODE_ID);
    ...
    pub fn deploy(&mut self, contract_hex: &str) {
        let params = evm::ConstructorParams {
            creator: EthAddress::from_id(fil_actors_runtime_v11::EAM_ACTOR_ADDR.id().unwrap()),
            initcode: hex::decode(contract_hex).unwrap().into(),
        };
        // invoke constructor
        self.runtime
            .expect_validate_caller_addr(vec![INIT_ACTOR_ADDR]);
        self.runtime
            .set_caller(*INIT_ACTOR_CODE_ID, INIT_ACTOR_ADDR);
