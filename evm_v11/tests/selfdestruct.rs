...
use fil_actors_runtime_v11::{test_utils::*, EAM_ACTOR_ADDR, INIT_ACTOR_ADDR};
...
fn test_selfdestruct() {
    ...
    // Calls still work.
    assert_eq!(
        U256::from_big_endian(&util::invoke_contract(&rt, &returnone_params)),
        U256::ONE
    );
    rt.verify();
    ...
    rt.set_caller(*EAM_ACTOR_CODE_ID, EAM_ACTOR_ADDR);
    rt.expect_validate_caller_addr(vec![EAM_ACTOR_ADDR]);
    rt.call::<EvmContractActor>(Method::Resurrect as MethodNum, resurrect_params)
        .unwrap();
    rt.verify();
