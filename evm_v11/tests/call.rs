...
use fil_actors_runtime_v11::{test_utils::*, EAM_ACTOR_ID, INIT_ACTOR_ADDR};
...
fn test_call() {
    ...
    let evm_target = EthAddress(hex_literal::hex!(
        "deadbeefdeadbeefdeadbeefdeadbeefdeadbeef"
    ));
    let f4_target: FILAddress = evm_target.try_into().unwrap();
    rt.actor_code_cids
        .borrow_mut()
        .insert(target, *EVM_ACTOR_CODE_ID);
...
fn test_reserved_method() {
    ...
    let code = rt
        .call::<evm::EvmContractActor>(0x42, None)
        .unwrap_err()
        .exit_code();
...
abigen!(
    CallActorPrecompile,
    "./tests/contracts/CallActorPrecompile.abi"
);
...
fn call_actor_overlapping() {
    ...
    test.run_test_expecting(
        &rt,
        CallActorReturn::default(),
        util::PrecompileExit::Success,
    );
}
...
mod call_actor_invalid {
    ...
    fn invalid_params_zero_codec() {
        ...
        call_params
            ...
            .set_addr(
                CallActorParams::EMPTY_PARAM_ADDR_OFFSET + 32,
                addr.to_bytes(),
            );
    ...
    fn invalid_params_zero_codec_2() {
        ...
        call_params
            .set_params(CallActorParams::FIRST_DYNAMIC_OFFSET, Some(send_params))
            .set_addr(
                CallActorParams::EMPTY_PARAM_ADDR_OFFSET + 1,
                addr.to_bytes(),
            );
