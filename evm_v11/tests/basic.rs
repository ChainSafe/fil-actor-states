...
fn basic_contract_construction_and_invocation_fe_lang() {
    let bytecode = hex::decode(include_str!(
        "contracts/output/FeSimplecoin/FeSimplecoin.bin"
    ))
    .unwrap();
...
fn simplecoin_test(bytecode: Vec<u8>) {
    ...
    let rt = util::init_construct_and_verify(bytecode, |rt| {
        rt.actor_code_cids
            .borrow_mut()
            .insert(contract, *EVM_ACTOR_CODE_ID);
...
"#;
        ...
        (
            asm::new_contract("get_bytecode", init, body).unwrap(),
            body_bytecode,
        )
    };
