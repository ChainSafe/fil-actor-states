...
pub(crate) fn is_dead(rt: &impl Runtime, state: &State) -> bool {
    state
        .tombstone
        .map_or(false, |t| t != current_tombstone(rt))
}
...
impl EvmContractActor {
        ...
        RT::Blockstore: Clone,
    {
        ...
        initialize_evm_contract(
            &mut System::create(rt)?,
            params.creator,
            params.initcode.into(),
        )
    }
        ...
        RT::Blockstore: Clone,
    {
        ...
        let bytecode_cid = match system.get_bytecode() {
            ...
            // an EVM contract with no code returns immediately
            None => {
                return Ok(InvokeContractReturn {
                    output_data: Vec::new(),
                })
            }
        };
        ...
        RT::Blockstore: Clone,
    {
        ...
        let params = args.unwrap_or(IpldBlock {
            codec: 0,
            data: vec![],
        });
...
fn handle_filecoin_method_input(method: u64, codec: u64, params: &[u8]) -> Vec<u8> {
    let static_args = [
        method,
        codec,
        EVM_WORD_SIZE as u64 * 3, /* start of params */
        params.len() as u64,
    ];
    let total_words =
...
fn handle_filecoin_method_output(output: &[u8]) -> Result<Option<IpldBlock>, ActorError> {
    ...
    let return_block = match codec {
        ...
        // Supported codecs.
        fvm_ipld_encoding::CBOR => Some(IpldBlock {
            codec,
