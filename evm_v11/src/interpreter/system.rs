...
use cid::Cid;
use fil_actors_runtime_v11::{runtime::Runtime, ActorError};
...
const KAMT_CONFIG: KamtConfig = KamtConfig {
    min_data_depth: 0,
    bit_width: 5,
    max_array_width: 1,
};
...
impl<'r, RT: Runtime> System<'r, RT> {
        ...
        RT::Blockstore: Clone,
    {
        ...
        if !crate::is_dead(rt, &state) {
            return Err(actor_error!(
                forbidden,
                "can only resurrect a dead contract"
            ));
        }
        ...
        RT::Blockstore: Clone,
    {
        ...
        if state_root != EMPTY_ARR_CID {
            return Err(actor_error!(
                illegal_state,
                "can't create over an existing actor"
            ));
        }
        return Ok(Self::new(rt, read_only));
    }
    ...
    pub fn send_raw(
        ...
        let result = self
            .rt
            .send(to, method, params, value, gas_limit, send_flags);
    ...
    pub fn flush(&mut self) -> Result<(), ActorError> {
        ...
        if self.readonly {
            return Err(ActorError::forbidden(
                "contract invocation is read only".to_string(),
            ));
        }
    ...
    pub fn get_randomness(&mut self) -> Result<&[u8; 32], ActorError> {
        ...
        match &mut self.randomness {
            ...
            cache => Ok(cache.insert(self.rt.get_randomness_from_beacon(
                fil_actors_runtime_v11::runtime::DomainSeparationTag::EvmPrevRandao,
                self.rt.curr_epoch(),
                ENTROPY,
            )?)),
        }
