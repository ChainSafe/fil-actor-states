...
impl Stack {
    ...
    pub fn pop(&mut self) -> Result<U256, ActorError> {
        self.stack
            .pop()
            .context_code(EVM_CONTRACT_STACK_UNDERFLOW, "stack underflow")
    }
...
fn test_stack_swap_underflow() {
    let mut stack = Stack::new();
    assert_eq!(
        stack.swap_top(1).unwrap_err().exit_code(),
        EVM_CONTRACT_STACK_UNDERFLOW
    );
    ...
    stack.push(2.into()).unwrap();
    assert_eq!(
        stack.swap_top(2).unwrap_err().exit_code(),
        EVM_CONTRACT_STACK_UNDERFLOW
    );
}
...
fn test_stack_dup_underflow() {
    let mut stack = Stack::new();
    assert_eq!(
        stack.dup(1).unwrap_err().exit_code(),
        EVM_CONTRACT_STACK_UNDERFLOW
    );
    stack.push(1.into()).unwrap();
    assert_eq!(
        stack.dup(2).unwrap_err().exit_code(),
        EVM_CONTRACT_STACK_UNDERFLOW
    );
}
...
fn test_stack_overflow() {
    ...
    assert_eq!(
        stack.push(1024.into()).unwrap_err().exit_code(),
        EVM_CONTRACT_STACK_OVERFLOW
    );
    assert_eq!(
        stack.dup(1).unwrap_err().exit_code(),
