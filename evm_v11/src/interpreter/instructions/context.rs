...
mod tests {
    ...
    fn test_blockhash() {
        ...
        let long_unknown = Cid::new_v1(
            IPLD_RAW,
            multihash::Multihash::wrap(0, &counting_byte_hash).unwrap(),
        );
        let long_expect = counting_byte_hash[..32].try_into().unwrap();
        // multihash code ignored
        let cbor_odd_hash = Cid::new_v1(
            DAG_CBOR,
            multihash::Multihash::wrap(123, &[0xfe; 32]).unwrap(),
        );
        ...
        for (current, getting, insert, expect, test) in [
            ...
            (
                ...
                None,
                nothing,
                "requested older than finality (256)",
            ),
        ] {
    ...
    fn test_prevrandao() {
        ...
        evm_unit_test! {
            (rt) {
                rt.set_epoch(epoch);
                rt.expect_get_randomness_from_beacon(fil_actors_runtime_v11::runtime::DomainSeparationTag::EvmPrevRandao, epoch, Vec::from(*b"prevrandao"), [0xff; 32]);
            }
            (m) {
                PREVRANDAO;
            }
