...
pub fn create(
    ...
    if system.readonly {
        return Err(ActorError::read_only(
            "create called while read-only".into(),
        ));
    }
    ...
    let params = eam::CreateParams {
        code: input_data.to_vec(),
        nonce,
    };
    create_init(
        system,
...
pub fn create2(
    ...
    if system.readonly {
        return Err(ActorError::read_only(
            "create2 called while read-only".into(),
        ));
    }
...
pub fn selfdestruct(
    ...
    if system.readonly {
        return Err(ActorError::read_only(
            "selfdestruct called while read-only".into(),
        ));
    }
    ...
    extract_send_result(
        system
            .rt
            .send_simple(&beneficiary, METHOD_SEND, None, balance),
    )
    .map_err(|e| {
...
mod tests {
    ...
    #[test]
    fn test_create() {
        let ret_addr = EthAddress(hex_literal::hex!(
