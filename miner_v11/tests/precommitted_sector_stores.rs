...
use fil_actors_runtime_v11::test_utils::make_sealed_cid;
...
fn put_get_and_delete() {
    ...
    let pc1 = new_pre_commit_on_chain(
        1,
        make_sealed_cid("1".as_bytes()),
    ...
    let pc2 = new_pre_commit_on_chain(
        2,
        make_sealed_cid("2".as_bytes()),
        TokenAmount::from_atto(1),
        1,
    );
    ...
    let pc3 = new_pre_commit_on_chain(
        3,
        make_sealed_cid("2".as_bytes()),
        TokenAmount::from_atto(1),
        1,
    );
    let pc4 = new_pre_commit_on_chain(
        4,
        make_sealed_cid("2".as_bytes()),
        TokenAmount::from_atto(1),
        1,
    );
    h.put_precommitted_sectors(vec![pc3.clone(), pc4.clone()])
        .unwrap();
