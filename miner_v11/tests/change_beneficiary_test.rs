...
fn successfully_change_owner_to_another_address_two_message() {
    ...
    h.change_beneficiary(&rt, h.owner, &beneficiary_change, None)
        .unwrap();
...
fn successfully_change_from_not_owner_beneficiary_to_another_address_three_message() {
    ...
    h.change_beneficiary(&rt, h.owner, &second_beneficiary_change, None)
        .unwrap();
    let mut beneficiary_return = h.get_beneficiary(&rt).unwrap();
    ...
    h.change_beneficiary(&rt, second_beneficiary_id, &second_beneficiary_change, None)
        .unwrap();
    beneficiary_return = h.get_beneficiary(&rt).unwrap();
    assert_eq!(first_beneficiary_id, beneficiary_return.active.beneficiary);
    ...
    h.change_beneficiary(&rt, first_beneficiary_id, &second_beneficiary_change, None)
        .unwrap();
    beneficiary_return = h.get_beneficiary(&rt).unwrap();
    assert_eq!(None, beneficiary_return.proposed);
    assert_eq!(
...
fn successfully_change_from_not_owner_beneficiary_to_another_address_when_beneficiary_inefficient_two_message(
    ...
    h.change_beneficiary(&rt, h.owner, &another_beneficiary_change, None)
        .unwrap();
...
fn successfully_owner_immediate_revoking_unapproved_proposal() {
    ...
    h.change_beneficiary(&rt, h.owner, &back_owner_beneficiary_change, Some(h.owner))
        .unwrap();
...
fn successfully_immediately_change_back_to_owner_address_while_used_up_quota() {
    ...
    h.withdraw_funds(&rt, h.beneficiary, &quota, &quota, &TokenAmount::zero())
        .unwrap();
    let back_owner_beneficiary_change =
        BeneficiaryChange::new(h.owner, TokenAmount::zero(), ChainEpoch::from(0));
    h.change_beneficiary(&rt, h.owner, &back_owner_beneficiary_change, Some(h.owner))
...
fn successfully_change_quota_and_test_withdraw() {
    ...
    let decrease_beneficiary_change = BeneficiaryChange::new(
        first_beneficiary_id,
        decrease_quota,
        beneficiary_term.expiration,
    );
    h.change_beneficiary(&rt, h.owner, &decrease_beneficiary_change, None)
        .unwrap();
    h.change_beneficiary(
        &rt,
        first_beneficiary_id,
    ...
    expect_abort_contains_message(
        ...
        "beneficiary expiration of epoch",
        ret,
    );
    ...
    h.change_beneficiary(&rt, h.owner, &increase_beneficiary_change, None)
        .unwrap();
    h.change_beneficiary(
        &rt,
        first_beneficiary_id,
...
fn successfully_get_beneficiary() {
    ...
    h.propose_approve_initial_beneficiary(&rt, first_beneficiary_id, beneficiary_term)
        .unwrap();
