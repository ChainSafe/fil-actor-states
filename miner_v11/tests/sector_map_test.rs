...
fn deadline_sector_map() {
    ...
    assert!(deadline_sector_map
        .check(
            partition_count * deadline_count,
            partition_count * deadline_count
        )
        .is_ok());
    ...
    assert!(deadline_sector_map
        .check(
            partition_count * deadline_count,
            partition_count * deadline_count + 1
        )
        .is_ok());
}
    ...
    (0..deadline_count).for_each(|deadline_index| {
        deadline_sector_map
            .add(
                ...
                deadline_index,
                0,
                create_bitfield_sequence(0, u64::MAX),
            )
            .unwrap();
    });
...
fn partition_sector_map_overflow() {
    ...
    (0..partition_count).for_each(|partition_index| {
        partition_sector_map
            .add(partition_index, create_bitfield_sequence(0, u64::MAX))
            .unwrap();
    });
...
fn deadline_sector_map_sorted() {
    ...
    assert!(deadline_sector_map
        .deadlines()
        .sorted()
        .eq(deadline_sector_map.deadlines()));
}
...
fn partition_sector_map_sorted() {
    ...
    assert!(partition_sector_map
        .partitions()
        .sorted()
        .eq(partition_sector_map.partitions()));
}
