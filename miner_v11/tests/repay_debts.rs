...
use fil_actors_runtime_v11::test_utils::{expect_abort_contains_message, EVM_ACTOR_CODE_ID};
...
fn pay_debt_entirely_from_balance() {
    ...
    h.repay_debts(&rt, &debt_to_repay, &TokenAmount::zero(), &fee_debt)
        .unwrap();
...
fn repay_debt_restricted_correctly() {
    ...
    rt.call::<Actor>(Method::RepayDebtExported as u64, None)
        .unwrap();
...
fn partially_repay_debt() {
    ...
    h.repay_debts(&rt, &debt_to_repay, &TokenAmount::zero(), &debt_to_repay)
        .unwrap();
