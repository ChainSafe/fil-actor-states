...
fn adds_bitfield_to_empty_queue() {
    ...
    queue
        .add_to_queue(epoch, &BitField::try_from_bits(values).unwrap())
        .unwrap();
...
fn pop_until_removes_and_returns_entries_before_and_including_target_epoch() {
    ...
    queue
        .add_to_queue_values(epoch3, [6, 7, 8].to_vec())
        .unwrap();
...
fn cuts_elements() {
    ...
    queue
        .add_to_queue_values(epoch1, [1, 2, 3, 4, 99].to_vec())
        .unwrap();
...
fn empty_bitfield_queue_with_quantizing(
    ...
    let cid = Amt::<(), _>::new_with_bit_width(&rt.store, bitwidth)
        .flush()
        .unwrap();
