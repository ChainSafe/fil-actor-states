...
use fil_actors_runtime_v11::test_utils::EVM_ACTOR_CODE_ID;
...
fn successfully_change_only_the_worker_address() {
    ...
    h.change_worker_address(&rt, new_worker, original_control_addresses.clone())
        .unwrap();
...
fn change_and_confirm_worker_address_restricted_correctly() {
    ...
    rt.call::<Actor>(Method::ChangeWorkerAddressExported as u64, params)
        .unwrap();
    ...
    // call the exported method
    rt.expect_validate_caller_addr(vec![h.owner]);
    rt.call::<Actor>(Method::ConfirmChangeWorkerAddressExported as u64, None)
        .unwrap();
    rt.verify();
...
fn change_cannot_be_overridden() {
    ...
    let effective_epoch = current_epoch + rt.policy().worker_key_change_delay;
    h.change_worker_address(&rt, new_worker_1, original_control_addresses.clone())
        .unwrap();
    ...
    h.change_worker_address(&rt, new_worker_2, original_control_addresses)
        .unwrap();
...
fn successfully_resolve_and_change_only_control_addresses() {
    ...
    h.change_worker_address(
        &rt,
        h.worker,
        vec![control_address_1, control_address_2_non_id],
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        vec![control_address_1, control_address_2]
    );
...
fn successfully_change_both_worker_and_control_addresses() {
    ...
    rt.set_epoch(current_epoch);
    let effective_epoch = current_epoch + rt.policy().worker_key_change_delay;
    h.change_worker_address(&rt, new_worker, vec![control_address_1, control_address_2])
        .unwrap();
...
fn fails_if_control_addresses_length_exceeds_maximum_limit() {
    ...
    let control_addresses = (0..=rt.policy().max_control_addresses as u64)
        .map(Address::new_id)
        .collect_vec();
