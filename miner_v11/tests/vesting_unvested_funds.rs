...
fn unlock_unvested_funds_leaving_bucket_with_non_zero_tokens() {
    ...
    let amount_unlocked = h
        .unlock_unvested_funds(vest_start, &TokenAmount::from_atto(39))
        .unwrap();
    ...
    assert_eq!(
        TokenAmount::zero(),
        h.unlock_vested_funds(vest_start).unwrap()
    );
    assert_eq!(
        TokenAmount::zero(),
        h.unlock_vested_funds(vest_start + 1).unwrap()
    );

    // expected to be zero due to unlocking of UNvested funds
    assert_eq!(
        TokenAmount::zero(),
        h.unlock_vested_funds(vest_start + 2).unwrap()
    );
    // expected to be partially unlocked already du to unlocking of UNvested funds
    assert_eq!(
        TokenAmount::from_atto(1),
        h.unlock_vested_funds(vest_start + 3).unwrap()
    );

    assert_eq!(
        TokenAmount::from_atto(20),
        h.unlock_vested_funds(vest_start + 4).unwrap()
    );
    assert_eq!(
        TokenAmount::from_atto(20),
        h.unlock_vested_funds(vest_start + 5).unwrap()
    );
    assert_eq!(
        TokenAmount::from_atto(20),
        h.unlock_vested_funds(vest_start + 6).unwrap()
    );

    assert_eq!(
        TokenAmount::zero(),
        h.unlock_vested_funds(vest_start + 7).unwrap()
    );
...
fn unlock_unvested_funds_leaving_bucket_with_zero_tokens() {
    ...
    let amount_unlocked = h
        .unlock_unvested_funds(vest_start, &TokenAmount::from_atto(40))
        .unwrap();
...
fn unlock_unvested_funds_value_greater_than_locked_funds() {
    ...
    let vest_sum = TokenAmount::from_atto(100);
    h.add_locked_funds(vest_start, &vest_sum, &vspec).unwrap();
    let unvested_funds = h
        .unlock_unvested_funds(vest_start, &TokenAmount::from_atto(200))
        .unwrap();
    assert_eq!(vest_sum, unvested_funds);
...
fn unlock_unvested_funds_when_there_are_vested_funds_in_the_table() {
    ...
    let vesting = h
        .store
        .get_cbor::<VestingFunds>(&h.st.vesting_funds)
        .unwrap()
        .unwrap();
    let mut epoch = 11;
