...
use fil_actors_runtime_v11::runtime::{Policy, Runtime};
...
fn add_sectors(
    ...
    let (faulty_power, recovery_power) = deadline
        .process_deadline_end(store, QUANT_SPEC, 0, sectors_root)
        .unwrap();
...
fn add_then_terminate(
    ...
    let removed_power = terminate_sectors(
        ...
        sectors.to_owned(),
        HashMap::from([
            (0, bitfield_from_slice(&[1, 3])),
            (1, bitfield_from_slice(&[6])),
        ]),
    )
    .unwrap();
...
fn terminate_proven_and_faulty() {
    ...
    let expected_power_loss = power_for_sectors(
        SECTOR_SIZE,
        &select_sectors(&sectors, &bitfield_from_slice(&[3])),
    );
    assert_eq!(
...
fn post_all_the_things() {
    ...
    let mut post_partitions = [PoStPartition {
        index: 2,
        skipped: BitField::default(),
    }];
