...
use fil_actors_runtime_v11::test_utils::make_piece_cid;
use fil_actors_runtime_v11::{runtime::Runtime, test_utils::expect_abort, DealWeight};
use fvm_shared::{
    bigint::{BigInt, Zero},
    clock::ChainEpoch,
    econ::TokenAmount,
...
fn prove_single_sector() {
    ...
    let sector_power = PowerPair {
        raw: StoragePower::from(h.sector_size as u64),
        qa: qa_power.clone(),
    };
    ...
    assert_eq!(
        HashMap::from([(quantized_expiration, vec![p_idx])]),
        d_queue
    );
...
fn prove_sectors_from_batch_pre_commit() {
    ...
    let conf = PreCommitBatchConfig {
        sector_deal_data: vec![
            ...
            SectorDealData {
                commd: Some(make_piece_cid(b"1")),
            },
            SectorDealData {
                commd: Some(make_piece_cid(b"2|3")),
            },
        ],
    ...
    // Prove the last
    {
        ...
        pcc.add_verified_deals(
            ...
            vec![verified_deal2, verified_deal3],
        );
        let sector = h
            .prove_commit_sector_and_confirm(
                &rt,
        ...
        assert_eq!(&full_deal_pledge, &sector.initial_pledge);
        assert_eq!(
            &no_deal_pledge + &full_deal_pledge + &full_deal_pledge,
            st.initial_pledge
        );
    }
}
...
fn prove_commit_aborts_if_pledge_requirement_not_met() {
    ...
    let st = h.get_state(&rt);
    rt.balance
        .replace(&st.pre_commit_deposits + &st.initial_pledge + &st.locked_funds);
...
fn drop_invalid_prove_commit_while_processing_valid_one() {
    ...
    h.next_sector_no += 1;
    let params_b = h.make_pre_commit_params(
        h.next_sector_no,
        *rt.epoch.borrow() - 1,
        expiration,
        vec![2],
    ...
    h.prove_commit_sector(&rt, &pre_commit_a, h.make_prove_commit_params(sector_no_a))
        .unwrap();
    h.prove_commit_sector(&rt, &pre_commit_b, h.make_prove_commit_params(sector_no_b))
        .unwrap();
    ...
    h.confirm_sector_proofs_valid(&rt, conf, vec![pre_commit_a, pre_commit_b])
        .unwrap();
    let st = h.get_state(&rt);
