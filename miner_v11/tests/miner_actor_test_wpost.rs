...
use fil_actors_runtime_v11::runtime::DomainSeparationTag;
...
fn invalid_submissions() {
    ...
    {
        rt.epoch
            .replace_with(|e| *e + rt.policy.wpost_challenge_window);
        let partition = miner::PoStPartition {
            index: pidx,
            skipped: make_empty_bitfield(),
        };
...
fn duplicate_proof_rejected_with_many_partitions() {
    ...
    let state = h.get_state(&rt);
    let (dlidx, _) = state
        .find_sector(&rt.policy, &rt.store, last_sector.sector_number)
        .unwrap();
    let dlinfo = h.advance_to_deadline(&rt, dlidx);

    {
        ...
        let sectors_to_prove: Vec<_> = (0..h.partition_size)
            .map(|i| sectors[i as usize].clone())
            .collect();
    ...
    }
    {
        ...
        let mut sectors_to_prove: Vec<_> = (0..h.partition_size)
            .map(|i| sectors[i as usize].clone())
            .collect();
...
fn successful_recoveries_recover_power() {
    ...
    let state = h.get_state(&rt);
    let (dlidx, pidx) = state
        .find_sector(&rt.policy, &rt.store, infos[0].sector_number)
        .unwrap();
    let mut bf = BitField::new();
    bf.set(infos[0].sector_number);
    h.declare_recoveries(&rt, dlidx, pidx, bf, TokenAmount::zero())
        .unwrap();
...
fn skipped_faults_adjust_power() {
    ...
    let (dlidx, pidx) = state
        .find_sector(&rt.policy, &rt.store, infos[0].sector_number)
        .unwrap();
    let (dlidx2, pidx2) = state
        .find_sector(&rt.policy, &rt.store, infos[1].sector_number)
        .unwrap();
    ...
    let partition = miner::PoStPartition {
        index: pidx,
        skipped: make_bitfield(&[infos1[0].sector_number]),
    };
    h.submit_window_post(
        &rt,
        &dlinfo,
    ...
    let partition = miner::PoStPartition {
        index: pidx2,
        skipped: make_bitfield(&[infos[1].sector_number]),
    };
    let params = miner::SubmitWindowedPoStParams {
        deadline: dlinfo.index,
        partitions: vec![partition],
...
fn skipping_all_sectors_in_a_partition_rejected() {
    ...
    h.advance_deadline(
        &rt,
        CronConfig::with_continued_faults_penalty(TokenAmount::zero()),
    );
    h.check_state(&rt);
}
...
fn skipped_recoveries_are_penalized_and_do_not_recover_power() {
    ...
    let partition = miner::PoStPartition {
        index: pidx,
        skipped: make_bitfield(&[infos[0].sector_number]),
    };
    h.submit_window_post(
        &rt,
        &dlinfo,
...
fn skipping_a_fault_from_the_wrong_partition_is_an_error() {
    ...
    // Skip to the due deadline.
    let state = h.get_state(&rt);
    let (dlidx0, pidx0) = state
        .find_sector(&rt.policy, &rt.store, infos[0].sector_number)
        .unwrap();
    let (dlidx1, pidx1) = state
        .find_sector(&rt.policy, &rt.store, infos[N - 1].sector_number)
        .unwrap();
...
fn cannot_dispute_posts_when_the_challenge_window_is_open() {
    ...
    // Dispute it.
    let params = miner::DisputeWindowedPoStParams {
        deadline: dlinfo.index,
        post_index: 0,
    };
...
fn can_submit_v1_proof_types_nv19() {
    ...
    for tc in tests {
        ...
        rt.policy
            ...
            .insert(RegisteredPoStProof::StackedDRGWindow32GiBV1);
        rt.policy
            .valid_post_proof_type
            .insert(RegisteredPoStProof::StackedDRGWindow32GiBV1P1);
        rt.epoch.replace(precommit_epoch);
        rt.balance.replace(BIG_BALANCE.clone());
