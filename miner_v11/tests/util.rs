...
use fil_actors_runtime_v11::runtime::{DomainSeparationTag, Policy, Runtime, RuntimePolicy};
use fil_actors_runtime_v11::{test_utils::*, BatchReturn, BatchReturnGen};
...
impl Default for HarnessOptions {
    ...
    fn default() -> Self {
        HarnessOptions {
            proving_period_offset: 0,
            use_v2_pre_commit_and_replica_update: false,
        }
    }
}
...
impl ActorHarness {
    ...
    pub fn new(proving_period_offset: ChainEpoch) -> ActorHarness {
        Self::new_with_options(HarnessOptions {
    ...
    pub fn new_runtime(&self) -> MockRuntime {
        ...
        rt.policy
            ...
            .insert(self.window_post_proof_type);
        rt.policy
            .valid_pre_commit_proof_type
            .insert(self.seal_proof_type);
        ...
        for addr in &self.control_addrs {
            rt.actor_code_cids
                .borrow_mut()
                .insert(*addr, *ACCOUNT_ACTOR_CODE_ID);
        }
    ...
    pub fn construct_and_verify(&self, rt: &MockRuntime) {
        ...
        for a in self.control_addrs.iter() {
            rt.actor_code_cids
                .borrow_mut()
                .insert(*a, *ACCOUNT_ACTOR_CODE_ID);
        }
    ...
    pub fn set_multiaddr_fail(&self, rt: &MockRuntime, new_multiaddrs: Vec<BytesDe>) {
        let params = ChangeMultiaddrsParams {
            new_multi_addrs: new_multiaddrs,
        };
    ...
    pub fn get_control_addresses(&self, rt: &MockRuntime) -> (Address, Address, Vec<Address>) {
        ...
        let value = result
            .unwrap()
            .deserialize::<GetControlAddressesReturn>()
            .unwrap();
    ...
    pub fn commit_and_prove_sectors_with_cfgs(
        ...
        for i in 0..num_sectors {
            ...
            let sector_deal_ids = deal_ids
                .get(i)
                .and_then(|ids| Some(ids.clone()))
                .unwrap_or_default();
    ...
    pub fn compact_sector_numbers_raw(
        ...
        let params = CompactSectorNumbersParams {
            mask_sector_numbers: bf,
        };
    ...
    pub fn pre_commit_sector_batch_v2(
        ...
        if self.options.use_v2_pre_commit_and_replica_update {
            ...
            let v1 = params
                ...
                .iter()
                .map(|s| {
                    deal_data.push(SectorDealData {
                        commd: s.unsealed_cid.0,
                    });
                    PreCommitSectorParams {
    ...
    fn pre_commit_sector_batch_inner(
        ...
        for sector in sectors.iter() {
            ...
            sector_deal_data.push(SectorDealData {
                commd: sector.unsealed_cid.0,
            });
        ...
        if any_deals {
            let vdparams = VerifyDealsForActivationParams {
                sectors: sector_deals,
            };
            let vdreturn = VerifyDealsForActivationReturn {
                sectors: sector_deal_data,
            };
            rt.expect_send_simple(
    ...
    pub fn pre_commit_sector_batch_and_get(
        ...
        let result = self
            .pre_commit_sector_batch(rt, params.clone(), conf, base_fee)
            .unwrap();
        ...
        params
            .sectors
            .iter()
            .map(|sector| self.get_precommit(rt, sector.sector_number))
            .collect()
    }
    ...
    pub fn on_deadline_cron(&self, rt: &MockRuntime, cfg: CronConfig) {
        ...
        power_delta += &cfg
            .detected_faults_power_delta
            .unwrap_or_else(PowerPair::zero);
        power_delta += &cfg
    ...
    pub fn dispute_window_post(
        ...
        let params = DisputeWindowedPoStParams {
            deadline: deadline.index,
            post_index: proof_index,
        };
        let result = rt.call::<Actor>(
    ...
    pub fn advance_and_submit_posts(&self, rt: &MockRuntime, sectors: &[SectorOnChainInfo]) {
        ...
        for sector in sectors {
            let (dlidx, _) = state
                .find_sector(&rt.policy, &rt.store, sector.sector_number)
                .unwrap();
        ...
        while deadlines.len() > 0 {
            match deadlines.get(&dlinfo.index) {
                ...
                Some(dl_sectors) => {
                    ...
                    let dl = dl_arr
                        .load_deadline(&rt.policy, &rt.store, dlinfo.index)
                        .unwrap();
    ...
    pub fn declare_faults(
        ...
        PowerPair {
            raw: expected_raw_delta,
            qa: expected_qa_delta,
        }
    }
...
fn immediately_vesting_funds(rt: &MockRuntime, state: &State) -> TokenAmount {
    let vesting = rt
        .store
        .get_cbor::<VestingFunds>(&state.vesting_funds)
        .unwrap()
        .unwrap();
...
fn make_fault_params_from_faulting_sectors(
    ...
    let declarations = keys
        .iter()
        .map(|k| declaration_map.remove(k).unwrap())
        .collect();
...
pub fn sectors_arr_mbs(
    ...
    let empty_array = Amt::<(), _>::new_with_bit_width(store, SECTORS_AMT_BITWIDTH)
        .flush()
        .unwrap();
...
pub fn sectors_arr<'a, BS: Blockstore>(
    ...
    let empty_array = Amt::<SectorOnChainInfo, _>::new_with_bit_width(&store, HAMT_BIT_WIDTH)
        .flush()
        .unwrap();
    let mut sectors = Sectors::load(store, &empty_array).unwrap();
    sectors.store(sectors_info).unwrap();
...
pub fn sectors_as_map(sectors: &[SectorOnChainInfo]) -> SectorsMap {
    sectors
        .iter()
        .map(|sector| (sector.sector_number, sector.to_owned()))
        .collect()
}
...
impl CronControl {
    ...
    pub fn pre_commit_to_start_cron(
        ...
        h.pre_commit_sector(rt, precommit_params, PreCommitConfig::default(), true)
            .unwrap();
...
pub fn select_sectors(sectors: &[SectorOnChainInfo], field: &BitField) -> Vec<SectorOnChainInfo> {
    ...
    let included = sectors
        .iter()
        .filter(|sector| to_include.remove(&sector.sector_number))
        .cloned()
        .collect();
