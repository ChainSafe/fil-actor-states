...
fn cron_on_inactive_state() {
    ...
    assert_eq!(
        PERIOD_OFFSET - rt.policy.wpost_proving_period,
...
fn sector_expires() {
    ...
    let (dl_idx, _) = st
        .find_sector(&rt.policy, &rt.store, sectors[0].sector_number)
        .unwrap();
...
fn detects_and_penalizes_faults() {
    ...
    let st = h.get_state(&rt);
    let (dl_idx, p_idx) = st
        .find_sector(&rt.policy, &rt.store, active_sectors[0].sector_number)
        .unwrap();
    ...
    h.advance_deadline(
        &rt,
        CronConfig {
            continued_faults_penalty: ongoing_penalty,
            ..Default::default()
        },
    );
...
fn test_cron_run_trigger_faults() {
    ...
    let st = h.get_state(&rt);
    let (dl_idx, _) = st
        .find_sector(&rt.policy, &rt.store, all_sectors[0].sector_number)
        .unwrap();
    ...
    let power_delta_claim = PowerPair {
        raw: pwr.raw.neg(),
        qa: pwr.qa.neg(),
    };
