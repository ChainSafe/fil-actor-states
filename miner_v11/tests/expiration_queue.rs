...
fn mk_bitfield<const N: usize>(keys: [u64; N]) -> BitField {
    MaybeBitField::from_iter(keys.iter().copied())
        .try_into()
        .unwrap()
}
...
fn active_power() -> PowerPair {
    PowerPair {
        raw: StoragePower::from(1 << 13),
        qa: StoragePower::from(1 << 14),
    }
}
fn faulty_power() -> PowerPair {
    PowerPair {
        raw: StoragePower::from(1 << 11),
        qa: StoragePower::from(1 << 12),
    }
}
...
fn added_sectors_can_be_popped_off_queue() {
    ...
    assert_eq!(
        set.active_power,
        power_for_sectors(SECTOR_SIZE, &sectors()[0..3])
    );
...
fn reschedules_sectors_as_faults() {
    ...
    let power_delta = queue
        .reschedule_as_faults(6, &sectors()[1..5], SECTOR_SIZE)
        .unwrap();
    assert_eq!(
        power_delta,
        power_for_sectors(SECTOR_SIZE, &sectors()[1..5])
    );
    ...
    assert_eq!(
        set.faulty_power,
        power_for_sectors(SECTOR_SIZE, &sectors()[1..2])
    );
    ...
    assert_eq!(
        set.faulty_power,
        power_for_sectors(SECTOR_SIZE, &sectors()[2..5])
    );
...
fn reschedules_all_sectors_as_faults() {
    ...
    assert_eq!(
        set.faulty_power,
        power_for_sectors(SECTOR_SIZE, &sectors()[0..2])
    );
    ...
    assert_eq!(
        set.faulty_power,
        power_for_sectors(SECTOR_SIZE, &sectors()[2..])
    );
...
fn reschedule_recover_restores_all_sector_stats() {
    ...
    let recovered = queue
        .reschedule_recovered(sectors()[1..5].to_vec(), SECTOR_SIZE)
        .unwrap();
    ...
    assert_eq!(
        set.active_power,
        power_for_sectors(SECTOR_SIZE, &sectors()[2..4])
    );
    assert_eq!(set.faulty_power, PowerPair::zero());
...
fn replaces_sectors_with_new_sectors() {
    ...
    let (removed, added, power_delta, pledge_delta) = queue
        .replace_sectors(&to_remove, &to_add, SECTOR_SIZE)
        .unwrap();
    ...
    assert_eq!(
        power_delta,
        &added_power - &power_for_sectors(SECTOR_SIZE, &to_remove)
    );
    assert_eq!(
        TokenAmount::from_atto(1002 + 1004 - 1000 - 1001 - 1003),
        pledge_delta
    );
    ...
    assert_eq!(
        set.active_power,
        power_for_sectors(SECTOR_SIZE, &sectors[2..3])
    );
    assert_eq!(set.faulty_power, PowerPair::zero());
...
fn removes_sectors() {
    ...
    let _ = queue
        .reschedule_as_faults(6, &sectors()[1..], SECTOR_SIZE)
        .unwrap();
    ...
    let (removed, recovering_power) = queue
        .remove_sectors(&rt.policy, &to_remove, &faults, &recovering, SECTOR_SIZE)
        .unwrap();
    ...
    assert_eq!(
        ...
        power_for_sectors(SECTOR_SIZE, &sectors()[3..6])
    );
    assert_eq!(
        recovering_power,
...
fn rescheduling_no_expirations_as_faults_leaves_the_queue_empty() {
    ...
    let _ = queue
        .reschedule_as_faults(15, &sectors(), SECTOR_SIZE)
        .unwrap();
...
fn rescheduling_no_sectors_as_recovered_leaves_the_queue_empty() {
    ...
    let mut queue = empty_expiration_queue_with_quantizing(&rt, QuantSpec { unit: 4, offset: 1 });
    let _ = queue
        .reschedule_recovered([].to_vec(), SECTOR_SIZE)
        .unwrap();
...
fn empty_expiration_queue_with_quantizing(
    ...
    let empty_array = Amt::<(), _>::new_with_bit_width(&rt.store, TEST_AMT_BITWIDTH)
        .flush()
        .unwrap();
