...
fn loads_for_proof_with_replacement() {
    ...
    let vec_sectors = sectors
        .load_for_proof(&bf_from_vec(vec![0, 1]), &bf_from_vec(vec![0]))
        .unwrap();
...
fn loads_for_proof_without_replacement() {
    ...
    let vec_sectors = sectors
        .load_for_proof(&bf_from_vec(vec![0, 1]), &BitField::new())
        .unwrap();
    assert_eq!(s0, vec_sectors[0]);
    assert_eq!(s1, vec_sectors[1]);
}
...
fn empty_proof() {
    ...
    let vec_sectors = sectors
        .load_for_proof(&BitField::new(), &BitField::new())
        .unwrap();
    assert_eq!(vec_sectors.len(), 0);
}
...
fn no_non_faulty_sectors() {
    ...
    let vec_sectors = sectors
        .load_for_proof(&bf_from_vec(vec![1]), &bf_from_vec(vec![1]))
        .unwrap();
    assert_eq!(vec_sectors.len(), 0);
}
