...
fn assert_simple_batch(
    ...
    for i in (0..batch_size).map(|i| i as usize) {
        ...
        assert_eq!(
            conf.sector_deal_data[i].commd,
            precommits[i].info.unsealed_cid.0
        );
...
mod miner_actor_precommit_batch {
    ...
    use fil_actors_runtime_v11::{STORAGE_MARKET_ACTOR_ADDR, STORAGE_POWER_ACTOR_ADDR};
    ...
    fn one_deal(v2: bool) {
        assert_simple_batch(
            ...
            &[DealSpec {
                ids: vec![1],
                commd: Some(make_piece_cid("1".as_bytes())),
            }],
            ExitCode::OK,
            "",
        );
    ...
    fn many_deals(v2: bool) {
        assert_simple_batch(
            ...
            &[
                ...
                DealSpec {
                    ids: vec![2],
                    commd: Some(make_piece_cid("2".as_bytes())),
                },
                DealSpec {
                    ids: vec![1, 2],
                    commd: Some(make_piece_cid("1|2".as_bytes())),
                },
            ],
            ExitCode::OK,
            "",
