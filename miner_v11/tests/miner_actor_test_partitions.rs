...
use fil_actors_runtime_v11::MessageAccumulator;
...
fn setup_unproven() -> (MockRuntime, Partition) {
    ...
    let power = partition
        .add_sectors(&rt.store, false, &sectors(), SECTOR_SIZE, QUANT_SPEC)
        .unwrap();
...
mod miner_actor_test_partitions {
    ...
    fn fails_to_add_faults_for_missing_sectors() {
        ...
        assert!(err
            .to_string()
            .contains("not all sectors are assigned to the partition"));
    }
    ...
    fn adds_recoveries() {
        ...
        partition
            .declare_faults_recovered(&sector_arr, SECTOR_SIZE, &recover_set)
            .unwrap();
    ...
    fn recovers_faults() {
        ...
        let recovered_power = partition
            .recover_faults(&rt.store, &sector_arr, SECTOR_SIZE, QUANT_SPEC)
            .unwrap();
    ...
    fn faulty_power_recovered_exactly_once() {
        ...
        let recovering_power =
            power_for_sectors(SECTOR_SIZE, &select_sectors(&sectors(), &fault_set));
        partition
            .declare_faults_recovered(&sector_arr, SECTOR_SIZE, &fault_set)
            .unwrap();
    ...
    fn replace_sectors_errors_when_attempting_to_replace_inactive_sector() {
        ...
        assert!(err
            .to_string()
            .contains("refusing to replace inactive sectors"));
    }
    ...
    fn pop_expiring_sectors() {
        ...
        let expire_epoch = 5;
        let expset = partition
            .pop_expired_sectors(&rt.store, expire_epoch, QUANT_SPEC)
            .unwrap();

        assert_bitfield_equals(&expset.on_time_sectors, &[1, 2]);
        assert_bitfield_equals(&expset.early_sectors, &[4]);
        assert_eq!(
            TokenAmount::from_atto(1000u64 + 1001),
            expset.on_time_pledge
        );
    ...
    fn pop_expiring_sectors_errors_if_a_recovery_exists() {
        ...
        assert!(err
            .to_string()
            .contains("unexpected recoveries while processing expirations"));
    }
    ...
    fn records_missing_post() {
        ...
        let (power_delta, penalized_power, _) = partition
            .record_missed_post(&rt.store, 6, QUANT_SPEC)
            .unwrap();
