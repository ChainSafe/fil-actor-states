...
fn updates_expiration_with_valid_params(v2: bool) {
    ...
    let (deadline_index, partition_index) = state
        .find_sector(rt.policy(), rt.store(), old_sector.sector_number)
        .unwrap();
    ...
    let expiration_set = partition
        .pop_expired_sectors(rt.store(), new_expiration - 1, quant)
        .unwrap();
...
fn update_expiration2_failure_cases() {
    ...
    /* Claim not found */
    {
        ...
        claims.insert(
            claim_ids[1],
            Err(actor_error!(not_found, "claim not found")),
        );
...
fn check_for_expiration(
    ...
    let (_, mut partition) = h.get_deadline_and_partition(rt, deadline_index, partition_index);
    let expiration_set = partition
        .pop_expired_sectors(rt.store(), expiration - 1, quant)
        .unwrap();
    assert!(expiration_set.is_empty());
