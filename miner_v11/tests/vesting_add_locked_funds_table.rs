...
fn test_vesting() {
    ...
    for tc in tests {
        ...
        h.st.add_locked_funds(
            &h.store,
            vest_start,
            &TokenAmount::from_atto(vest_sum),
            &tc.vspec,
        )
        .unwrap();
        ...
        for (e, &v) in tc.vepocs.iter().enumerate() {
            assert_eq!(
                ...
                h.st.unlock_vested_funds(&h.store, vest_start + e as ChainEpoch)
                    .unwrap()
            );
            total_vested += v;
            assert_eq!(
                TokenAmount::from_atto(vest_sum - total_vested),
                h.st.locked_funds
            );
        }
