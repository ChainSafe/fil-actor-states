...
fn withdraw_only_what_we_can_after_fee_debt() {
    ...
    h.withdraw_funds(&rt, h.owner, &requested, &expected_withdraw, &fee_debt)
        .unwrap();
    h.check_state(&rt);
}
...
fn successfully_withdraw() {
    ...
    h.withdraw_funds(&rt, h.owner, &one, &one, &TokenAmount::zero())
        .unwrap();
    ...
    h.withdraw_funds(&rt, h.owner, &one, &one, &TokenAmount::zero())
        .unwrap();
    h.withdraw_funds(&rt, h.beneficiary, &one, &one, &TokenAmount::zero())
        .unwrap();
    h.check_state(&rt);
}
...
fn fail_withdraw_from_non_beneficiary() {
    ...
    h.withdraw_funds(&rt, first_beneficiary_id, &one, &one, &TokenAmount::zero())
        .unwrap();
    h.check_state(&rt);
}
