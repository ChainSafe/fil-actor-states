...
fn info_getters() {
    ...
    let is_control_ret: IsControllingAddressReturn = rt
        .call::<Actor>(
            Method::IsControllingAddressExported as u64,
            IpldBlock::serialize_cbor(&IsControllingAddressParam {
                address: INIT_ACTOR_ADDR,
            })
            .unwrap(),
        )
        .unwrap()
...
fn collateral_getters() {
    ...
    assert_eq!(
        actor_balance.sub(sector.initial_pledge),
        available_balance_ret.available_balance
    );
