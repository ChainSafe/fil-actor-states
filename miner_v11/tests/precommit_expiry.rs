...
mod add_precommit_expiry {
    ...
    fn simple_pre_commit_expiry_and_cleanup() {
        ...
        h.add_pre_commit_clean_ups(&policy, Vec::from([(100, 1)]))
            .unwrap();
        ...
        h.add_pre_commit_clean_ups(&policy, Vec::from([(100, 2)]))
            .unwrap();
        BitFieldQueueExpectation::default()
            .add(quant.quantize_up(100), &[1, 2])
            .equals(&h.load_pre_commit_clean_ups(&policy));

        h.add_pre_commit_clean_ups(&policy, Vec::from([(200, 3)]))
            .unwrap();
        BitFieldQueueExpectation::default()
            .add(quant.quantize_up(100), &[1, 2])
    ...
    fn batch_pre_commit_expiry() {
        ...
        h.add_pre_commit_clean_ups(&policy, Vec::from([(100, 1), (200, 2), (200, 3)]))
            .unwrap();
