...
use fil_actors_runtime_v11::{runtime::Policy, ActorError};
...
impl StateHarness {
    ...
    pub fn new_with_policy(policy: &Policy, period_boundary: ChainEpoch) -> Self {
        ...
        let info = MinerInfo::new(
            ...
            vec![
                BytesDe("foobar".as_bytes().to_vec()),
                BytesDe("imafilminer".as_bytes().to_vec()),
            ],
    ...
    pub fn delete_precommitted_sectors(
        ...
        self.st
            .delete_precommitted_sectors(&self.store, sector_nums)
    }

    #[allow(dead_code)]
    pub fn get_precommit(&self, sector_number: SectorNumber) -> SectorPreCommitOnChainInfo {
        self.st
            .get_precommitted_sector(&self.store, sector_number)
            .unwrap()
            .unwrap()
    }
    ...
    pub fn add_pre_commit_clean_ups(
        ...
        self.st
            .add_pre_commit_clean_ups(policy, &self.store, cleanup_events)
    }
    ...
    pub fn add_locked_funds(
        ...
        self.st
            .add_locked_funds(&self.store, current_epoch, vesting_sum, spec)
    }
    ...
    pub fn unlock_unvested_funds(
        ...
        self.st
            .unlock_unvested_funds(&self.store, current_epoch, target)
    }
    ...
    pub fn put_sector(&mut self, sector: &SectorOnChainInfo) {
        self.st
            .put_sectors(&self.store, vec![sector.clone()])
            .unwrap();
    }

    pub fn get_sector(&self, sector_number: SectorNumber) -> SectorOnChainInfo {
        self.st
            .get_sector(&self.store, sector_number)
            .unwrap()
            .unwrap()
    }
    ...
    pub fn vesting_funds_store_empty(&self) -> bool {
        let vesting = self
            .store
            .get_cbor::<VestingFunds>(&self.st.vesting_funds)
            .unwrap()
            .unwrap();
    ...
    pub fn mask_sector_numbers(&mut self, ns: &BitField) -> Result<(), ActorError> {
        self.st
            .allocate_sector_numbers(&self.store, ns, CollisionPolicy::AllowCollisions)
    }
    ...
    pub fn expect_allocated_sector_numbers(&mut self, expected: &BitField) {
        let b: BitField = self
            .store
            .get_cbor(&self.st.allocated_sectors)
            .unwrap()
            .unwrap();
