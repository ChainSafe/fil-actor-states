...
use fil_actors_runtime_v11::cbor::{serialize, serialize_vec};
use fil_actors_runtime_v11::runtime::builtins::Type;
use fil_actors_runtime_v11::runtime::{ActorCode, DomainSeparationTag, Policy, Runtime};
...
impl Actor {
    pub fn constructor(
        ...
        let owner = rt.resolve_address(&params.owner).ok_or_else(|| {
            actor_error!(
                illegal_argument,
                "unable to resolve owner address: {}",
                params.owner
            )
        })?;
        ...
        let control_addresses: Vec<_> = params
            ...
            .map(|address| {
                rt.resolve_address(&address).ok_or_else(|| {
                    actor_error!(
                        illegal_argument,
                        "unable to resolve control address: {}",
                        address
                    )
                })
            })
    ...
    fn get_owner(rt: &impl Runtime) -> Result<GetOwnerReturn, ActorError> {
        ...
        Ok(GetOwnerReturn {
            owner: info.owner,
            proposed: info.pending_owner_address,
        })
    }
    ...
    fn is_controlling_address(
        ...
        let input = match rt.resolve_address(&params.address) {
            ...
            None => {
                return Ok(IsControllingAddressReturn {
        ...
        RT: Runtime,
    {
        ...
        let succeeded_sectors = rt.transaction(|state: &mut State, rt| {
            ...
            for &dl_idx in deadlines_to_load.iter() {
                ...
                for with_details in &decls_by_deadline[&dl_idx] {
                    let update_proof_type = with_details
                        ...
                        .map_err(|_| {
                            actor_error!(illegal_state, "couldn't load update proof type")
                        })?;
                    ...
                    new_sector_info.expected_day_reward = expected_reward_for_power(
                        ...
                        &qa_pow,
                        fil_actors_runtime_v11::network::EPOCHS_IN_DAY,
                    );
            ...
            state
                .check_balance_invariants(&current_balance)
                .map_err(balance_invariants_broken)?;
...
fn enroll_cron_event(
    ...
    let ser_params = IpldBlock::serialize_cbor(&ext::power::EnrollCronEventParams {
        event_epoch,
        payload,
    })?;
    extract_send_result(rt.send_simple(
    ...
    let (total_pledge, newly_vested) = rt.transaction(|state: &mut State, rt| {
        ...
        for (pre_commit, deal_spaces) in valid_pre_commits {
            ...
            let day_reward = expected_reward_for_power(
                ...
                quality_adj_power_smoothed,
                &power,
                fil_actors_runtime_v11::EPOCHS_IN_DAY,
            );
