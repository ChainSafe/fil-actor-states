...
use fil_actors_runtime_v11::VERIFIED_REGISTRY_ACTOR_ADDR;
...
mod construction {
    ...
    fn construct_with_verified() {
        let rt = new_runtime();
        let h = Harness {
            governor: VERIFIED_REGISTRY_ACTOR_ADDR,
        };
...
mod mint {
    ...
    use fil_actors_runtime_v11::test_utils::{expect_abort_contains_message, MARKET_ACTOR_CODE_ID};
    use fil_actors_runtime_v11::{STORAGE_MARKET_ACTOR_ADDR, VERIFIED_REGISTRY_ACTOR_ADDR};
    ...
    fn auto_allowance_on_mint() {
        ...
        h.mint(&rt, &ALICE, &TokenAmount::from_whole(1), vec![*BOB])
            .unwrap();
        let allowance = h.get_allowance_between(&rt, &ALICE, &BOB);
        assert!(allowance.eq(&INFINITE_ALLOWANCE));
...
mod transfer {
    ...
    use fil_actors_runtime_v11::test_utils::expect_abort_contains_message;
    use fvm_ipld_encoding::RawBytes;
    use fvm_shared::econ::TokenAmount;
    use fvm_shared::error::ExitCode;
    ...
    fn only_governor_allowed() {
        ...
        h.transfer(&rt, &ALICE, &h.governor, &amt, operator_data.clone())
            .unwrap();

        // The governor can transfer out.
        h.transfer(&rt, &h.governor, &BOB, &amt, operator_data)
            .unwrap();
    }
