...
use fil_actors_runtime_v11::FIRST_EXPORTED_METHOD_NUMBER;
use fil_actors_runtime_v11::{INIT_ACTOR_ADDR, SYSTEM_ACTOR_ADDR};
...
mod vesting_tests {
    ...
    fn happy_path_full_vesting() {
        ...
        h.construct_and_verify(
            &rt,
            2,
            UNLOCK_DURATION,
            START_EPOCH,
            vec![ANNE, BOB, CHARLIE],
        );
        ...
        expect_abort(
            ...
            h.approve(&rt, TxnID(0), proposal_hash),
        );
        rt.reset();
...
fn test_remove_signer() {
    ...
    for tc in test_cases {
        ...
        match tc.code {
            ...
            _ => assert_eq!(
                tc.code,
                ret.expect_err("remove signer return expected to be actor error")
                    .exit_code()
            ),
        }
        rt.verify();
...
fn test_signer_swap() {
    ...
    for tc in test_cases {
        ...
        match tc.code {
            ...
            _ => assert_eq!(
                tc.code,
                ret.expect_err("swap signer return expected to be actor error")
                    .exit_code()
            ),
        };
        check_state(&rt);
...
fn test_swap_signer_removes_approvals() {
    ...
    // anne proposes a tx
    rt.set_caller(*ACCOUNT_ACTOR_CODE_ID, anne);
    let proposal_hash1 = h.propose_ok(
        &rt,
        chuck,
        TokenAmount::zero(),
        METHOD_SEND,
    ...
    // bob proposes a tx
    rt.set_caller(*ACCOUNT_ACTOR_CODE_ID, bob);
    let proposal_hash2 = h.propose_ok(
        &rt,
        chuck,
        TokenAmount::zero(),
        METHOD_SEND,
...
mod approval_tests {
    ...
    fn proposed_tx_is_approved_if_number_approvers_has_crossed_threshold() {
        ...
        rt.set_caller(*MULTISIG_ACTOR_CODE_ID, msig);
        let new_threshold = 1;
        h.change_num_approvals_threshold(&rt, new_threshold)
            .unwrap();
...
mod cancel_tests {
    ...
    fn test_signer_fails_to_cancel_transaction_from_another_signer() {
        ...
        expect_abort(
            ExitCode::USR_FORBIDDEN,
            h.cancel(&rt, TxnID(0), proposal_hash),
        );
        rt.reset();
    ...
    fn fail_cancel_a_tx_that_does_not_exist() {
        ...
        expect_abort(
            ...
            h.cancel(&rt, dne_txn_id, proposal_hash),
        );
        rt.reset();
...
mod lock_balance_tests {
    ...
    fn retroactive_vesting() {
        ...
        rt.set_caller(*MULTISIG_ACTOR_CODE_ID, msig);
        h.lock_balance(&rt, vest_start, vest_duration, lock_amount.clone())
            .unwrap();
...
fn test_fallback() {
    ...
    rt.expect_validate_caller_any();
    let result = rt
        .call::<MultisigActor>(FIRST_EXPORTED_METHOD_NUMBER, params.clone())
        .unwrap();
    assert!(result.is_none());

    rt.expect_validate_caller_any();
    let result = rt
        .call::<MultisigActor>(FIRST_EXPORTED_METHOD_NUMBER + 1, params.clone())
        .unwrap();
    assert!(result.is_none());
