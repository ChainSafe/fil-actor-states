...
use fil_actors_runtime_v11::INIT_ACTOR_ADDR;
...
mod paych_constructor {
    ...
    fn create_paych_actor_after_resolving_to_id_address() {
        ...
        rt.actor_code_cids
            .borrow_mut()
            .insert(payee_addr, *ACCOUNT_ACTOR_CODE_ID);

        rt.id_addresses
            .borrow_mut()
            .insert(payer_non_id, payer_addr);
        rt.id_addresses
            .borrow_mut()
            .insert(payee_non_id, payee_addr);
...
mod create_lane_tests {
    ...
    fn create_lane_test() {
        ...
        for test_case in test_cases {
            ...
            rt.id_addresses
                .borrow_mut()
                .insert(paych_non_id, paych_addr);
...
mod update_channel_state_redeem {
    ...
    fn redeem_voucher_one_lane() {
        ...
        let exp_ls = LaneState {
            redeemed: TokenAmount::from_atto(9),
            nonce: 2,
        };
...
fn expect_authenticate_message(
    ...
    rt.expect_send(
        ...
        exp_exit_code
            .is_success()
            .then(|| AUTHENTICATE_MESSAGE_RESPONSE.clone())
            .flatten(),
        exp_exit_code,
        None,
    )
