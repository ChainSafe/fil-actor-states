...
use cid::Cid;
use fil_actors_runtime_v11::{extract_send_result, FIRST_ACTOR_SPECIFIC_EXIT_CODE};
...
use fil_actors_runtime_v11::runtime::{ActorCode, Policy, Runtime};
...
impl Actor {
    ...
    fn publish_storage_deals(
        ...
        for (di, mut deal) in params.deals.into_iter().enumerate() {
            ...
            let mut client_lockup = total_client_lockup
                .get(&client_id)
                .cloned()
                .unwrap_or_default();
            ...
            if deal.proposal.verified_deal {
                ...
                client_alloc_reqs.entry(client_id).or_default().push((
                    pcid,
                    alloc_request_for_deal(&deal.proposal, rt.policy(), curr_epoch),
                ));
            }
    ...
    fn get_deal_data_commitment(
        ...
        Ok(GetDealDataCommitmentReturn {
            data: found.piece_cid,
            size: found.piece_size,
        })
    }
    ...
    fn get_deal_client(
        ...
        rt.validate_immediate_caller_accept_any()?;
        let found = rt.state::<State>()?.get_proposal(rt.store(), params.id)?;
        Ok(GetDealClientReturn {
            client: found.client.id().unwrap(),
        })
    }
    ...
    fn get_deal_provider(
        ...
        Ok(GetDealProviderReturn {
            provider: found.provider.id().unwrap(),
        })
    }
    ...
    fn get_deal_term(
        ...
        rt.validate_immediate_caller_accept_any()?;
        let found = rt.state::<State>()?.get_proposal(rt.store(), params.id)?;
        Ok(GetDealTermReturn {
            start: found.start_epoch,
    ...
    fn get_deal_total_price(
        ...
        rt.validate_immediate_caller_accept_any()?;
        let found = rt.state::<State>()?.get_proposal(rt.store(), params.id)?;
        Ok(GetDealTotalPriceReturn {
            total_price: found.total_storage_fee(),
        })
    }
    ...
    fn get_deal_client_collateral(
        ...
        rt.validate_immediate_caller_accept_any()?;
        let found = rt.state::<State>()?.get_proposal(rt.store(), params.id)?;
        Ok(GetDealClientCollateralReturn {
            collateral: found.client_collateral,
        })
    }
    ...
    fn get_deal_provider_collateral(
        ...
        rt.validate_immediate_caller_accept_any()?;
        let found = rt.state::<State>()?.get_proposal(rt.store(), params.id)?;
        Ok(GetDealProviderCollateralReturn {
            collateral: found.provider_collateral,
        })
    }
    ...
    fn get_deal_verified(
        ...
        rt.validate_immediate_caller_accept_any()?;
        let found = rt.state::<State>()?.get_proposal(rt.store(), params.id)?;
        Ok(GetDealVerifiedReturn {
            verified: found.verified_deal,
        })
    }
...
fn compute_data_commitment(
    ...
    rt.compute_unsealed_sector_cid(sector_type, &pieces)
        .map_err(|e| {
            e.downcast_default(
...
fn alloc_request_for_deal(
    ...
    let alloc_expiration = min(
        deal.start_epoch,
        curr_epoch + policy.maximum_verified_allocation_expiration,
    );
    ext::verifreg::AllocationRequest {
...
fn datacap_transfer_request(
    ...
    Ok(TransferFromParams {
        ...
        operator_data: serialize(
            &ext::verifreg::AllocationRequests {
                allocations: alloc_reqs,
                extensions: vec![],
            },
            "allocation requests",
        )?,
...
fn deal_proposal_is_internally_valid(
    ...
    .context("proposal authentication failed")?
    {
        Err(actor_error!(
            illegal_argument,
            "proposal authentication failed"
        ))
    } else {
        Ok(())
    }
