...
use fil_actors_runtime_v11::cbor::{deserialize, serialize};
use fil_actors_runtime_v11::network::EPOCHS_IN_DAY;
use fil_actors_runtime_v11::runtime::{builtins::Type, Policy, Runtime, RuntimePolicy};
...
fn test_remove_all_error() {
    ...
    let rt = MockRuntime {
        receiver: market_actor,
        ..Default::default()
    };

    SetMultimap::new(&rt.store())
        .remove_all(42)
...
fn simple_construction() {
    ...
    assert!(rt
        .call::<MarketActor>(METHOD_CONSTRUCTOR, None)
        .unwrap()
        .is_none());
    ...
    let empty_balance_table = make_empty_map::<_, TokenAmount>(store, BALANCE_TABLE_BITWIDTH)
        .flush()
        .unwrap();
...
fn label_cbor() {
    let label = Label::String(
        "i_am_random_string____i_am_random_string____"
            .parse()
            .unwrap(),
    );
    let _ = to_vec(&label)
...
fn label_from_cbor() {
    ...
    let out = deserialize::<Label>(
        &RawBytes::from(bad_bytes),
        "cbor array, unexpected major type",
    );
...
fn balance_after_withdrawal_must_always_be_greater_than_or_equal_to_locked_amount() {
    ...
    let provider_acct = get_balance(&rt, &PROVIDER_ADDR);
    assert_eq!(
        &deal.provider_collateral + &withdrawable_amount,
        provider_acct.balance
    );
    assert_eq!(deal.provider_collateral, provider_acct.locked);
    ...
    let client_acct = get_balance(&rt, &CLIENT_ADDR);
    assert_eq!(
        deal.client_balance_requirement() + &withdrawable_amount,
        client_acct.balance
    );
    assert_eq!(deal.client_balance_requirement(), client_acct.locked);
...
fn adds_to_non_provider_funds() {
    ...
    for caller_addr in &[CLIENT_ADDR, WORKER_ADDR] {
        ...
        for tc in &test_cases {
            ...
            assert_eq!(
                get_balance(&rt, caller_addr).balance,
...
fn deal_starts_on_day_boundary() {
    ...
    let rt = setup_with_policy(Policy {
...
fn deal_expires() {
    ...
    assert!(DealArray::load(&st.proposals, &rt.store)
        .unwrap()
        .get(deal_id)
        .unwrap()
        .is_none());
...
fn provider_and_client_addresses_are_resolved_before_persisting_state_and_sent_to_verigreg_actor_for_a_verified_deal(
    ...
    rt.actor_code_cids
        .borrow_mut()
        .insert(client_resolved, *ACCOUNT_ACTOR_CODE_ID);
    rt.actor_code_cids
        .borrow_mut()
        .insert(provider_resolved, *MINER_ACTOR_CODE_ID);

    // mappings for resolving address
    rt.id_addresses
        .borrow_mut()
        .insert(client_bls, client_resolved);
    rt.id_addresses
        .borrow_mut()
        .insert(provider_bls, provider_resolved);
    ...
    assert_eq!(
        ...
        get_balance(&rt, &client_resolved).balance
    );
    ...
    assert_eq!(
        deal.provider_collateral,
        get_balance(&rt, &provider_resolved).balance
    );
...
fn publish_multiple_deals_for_different_clients_and_ensure_balances_are_correct() {
    ...
    assert_eq!(
        provider_locked_expected,
        get_balance(&rt, &PROVIDER_ADDR).locked
    );
    ...
    let addrs = MinerAddresses {
        provider: provider2_addr,
        ..MinerAddresses::default()
    };
    let deal6 =
    ...
    assert_eq!(
        provider_locked_expected + provider2_locked,
        st.total_provider_locked_collateral
    );
    let total_storage_fee =
...
fn cron_reschedules_update_to_new_period() {
    ...
    st.remove_deals_by_epoch(rt.store(), &[expected_epoch])
        .unwrap();
    st.put_deals_by_epoch(rt.store(), &[(misscheduled_epoch, deal_id)])
        .unwrap();
    rt.replace_state(&st);
...
fn cron_reschedules_many_updates() {
    ...
    assert_eq!(
        0,
        st.get_deals_for_epoch(rt.store(), start_epoch)
            .unwrap()
            .len()
    );
    ...
    let rescheduled = st
        .get_deals_for_epoch(rt.store(), start_epoch + update_interval)
        .unwrap();
    ...
    for epoch in (start_epoch + 1)..(start_epoch + update_interval) {
        ...
        assert_eq!(
            2,
            st.get_deals_for_epoch(rt.store(), epoch + update_interval)
                .unwrap()
                .len()
        );
    }
...
fn locked_fund_tracking_states() {
    ...
    rt.actor_code_cids
        .borrow_mut()
        .insert(p1, *MINER_ACTOR_CODE_ID);
    rt.actor_code_cids
        .borrow_mut()
        .insert(c1, *ACCOUNT_ACTOR_CODE_ID);
    let st: State = rt.get_state();
...
fn insufficient_client_balance_in_a_batch() {
    ...
    let provider_funds = deal1
        .provider_balance_requirement()
        .add(deal2.provider_balance_requirement());
    rt.set_caller(*ACCOUNT_ACTOR_CODE_ID, OWNER_ADDR);
    ...
    assert_eq!(
        deal2.client_balance_requirement(),
        get_balance(&rt, &CLIENT_ADDR).balance
    );
    assert_eq!(
        deal1
...
fn insufficient_provider_balance_in_a_batch() {
    ...
    add_participant_funds(
        ...
        CLIENT_ADDR,
        deal1
            .client_balance_requirement()
            .add(deal2.client_balance_requirement()),
    );
