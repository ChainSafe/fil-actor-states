...
pub fn expect_get_control_addresses(
    ...
    let result = GetControlAddressesReturnParams {
        owner,
        worker,
        control_addresses: controls,
    };
...
pub fn update_last_updated(rt: &MockRuntime, deal_id: DealID, new_last_updated: ChainEpoch) {
    ...
    states
        .set(
            deal_id,
            DealState {
                last_updated_epoch: new_last_updated,
                ..s
            },
        )
        .unwrap();
...
pub fn publish_deals(
    ...
    rt.expect_validate_caller_any();
    let return_value = ext::miner::IsControllingAddressReturn {
        is_controlling: true,
    };
    rt.expect_send_simple(
        addrs.provider,
    ...
    for deal in publish_deals {
        if deal.verified_deal {
            ...
            let cvd = client_verified_deals
                .entry(client_id)
                .or_insert(ClientVerifiedDeals {
                    deals: vec![],
    ...
    for deal in valid_deals {
        let buf = RawBytes::serialize(deal.clone()).expect("failed to marshal deal proposal");
        let params = IpldBlock::serialize_cbor(&MarketNotifyDealParams {
            proposal: buf.to_vec(),
            deal_id,
        })
        .unwrap();
    ...
    BS: fvm_ipld_blockstore::Blockstore,
{
    ...
    dobe.for_each(epoch, |id| {
        assert_eq!(
            epoch % deal_updates_interval,
            (id as i64) % deal_updates_interval
        );
        count += 1;
        Ok(())
    })
...
pub fn assert_deal_deleted(rt: &MockRuntime, deal_id: DealID, p: DealProposal) {
    ...
    use fil_actors_runtime_v11::Map;
    ...
    let p_cid = Cid::new_v1(
        ...
        mh_code.digest(&to_vec(&p).unwrap()),
    );
    ...
    let pending_deals: Map<fvm_ipld_blockstore::MemoryBlockstore, DealProposal> =
        fil_actors_runtime_v11::make_map_with_root_and_bitwidth::<
            fvm_ipld_blockstore::MemoryBlockstore,
            DealProposal,
        >(&st.pending_proposals, &*rt.store, PROPOSALS_AMT_BITWIDTH)
        .unwrap();
    assert!(!pending_deals
        .contains_key(&BytesKey(p_cid.to_bytes()))
        .unwrap());
}
    ...
    F: Fn(usize) -> Option<Vec<PieceInfo>>,
{
    ...
    let param = VerifyDealsForActivationParams {
