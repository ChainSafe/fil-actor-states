...
fn fail_when_client_has_some_funds_but_not_enough_for_a_deal() {
    ...
    add_provider_funds(
        &rt,
        deal1.clone().provider_collateral,
        &MinerAddresses::default(),
    );
...
fn fail_when_deals_have_different_providers() {
    ...
    let notify_param1 = IpldBlock::serialize_cbor(&MarketNotifyDealParams {
        proposal: RawBytes::serialize(&deal1)
            .expect("failed to marshal deal proposal")
            .to_vec(),
...
fn fails_if_notify_deal_fails() {
    ...
    rt.expect_send_simple(
        ...
        .unwrap(),
        TokenAmount::zero(),
        IpldBlock::serialize_cbor(&ext::miner::IsControllingAddressReturn {
            is_controlling: true,
        })
        .unwrap(),
        ExitCode::OK,
