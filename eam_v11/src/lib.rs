...
fn hash_20(rt: &impl Runtime, data: &[u8]) -> [u8; 20] {
    rt.hash(SupportedHashes::Keccak256, data)[12..32]
        .try_into()
        .unwrap()
}
...
fn create_actor(
    ...
    if !can_assign_address(&new_addr) {
        return Err(ActorError::forbidden(
            "cannot create address with a reserved prefix".into(),
        ));
    }

    let constructor_params = RawBytes::serialize(ext::evm::ConstructorParams {
        creator,
    ...
    if let Some(id) = rt.resolve_address(&f4_addr) {
        ...
        let caller_code_cid = rt
            ...
            .expect("failed to lookup actor code");
