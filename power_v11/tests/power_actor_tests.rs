...
use fil_actors_runtime_v11::runtime::builtins::Type;
...
use fil_actors_runtime_v11::{runtime::Policy, INIT_ACTOR_ADDR};
...
fn enroll_cron_epoch_multiple_events() {
    ...
    h.create_miner_basic(&rt, *OWNER, *OWNER, miner2_address)
        .unwrap();
    ...
    let enroll_and_check_cron_event = |epoch, miner_address, payload| {
        ...
        h.enroll_cron_event(&rt, epoch, miner_address, payload)
            .unwrap();
...
fn enroll_cron_epoch_before_current_epoch() {
    ...
    let miner_epoch = 2;
    let payload = RawBytes::serialize(b"Cthulhu").unwrap();
    h.enroll_cron_event(&rt, miner_epoch, &MINER, &payload)
        .unwrap();
...
fn new_miner_updates_miner_above_min_power_count() {
    ...
    let test_cases = [
        TestCase {
            proof: RegisteredPoStProof::StackedDRGWindow2KiBV1,
            expected_miners: 0,
        },
        TestCase {
            proof: RegisteredPoStProof::StackedDRGWindow32GiBV1,
            expected_miners: 0,
        },
    ];
...
fn get_network_and_miner_power() {
    ...
    let miner_power: MinerRawPowerReturn = rt
        .call::<Actor>(
            ...
            IpldBlock::serialize_cbor(&MinerRawPowerParams {
                miner: MINER1.id().unwrap(),
            })
            .unwrap(),
        )
        .unwrap()
...
mod cron_tests {
    ...
    fn call_reward_actor() {
        ...
        rt.call::<PowerActor>(Method::OnEpochTickEnd as u64, None)
            .unwrap();
    ...
    fn event_scheduled_in_null_round_called_next_round() {
        ...
        rt.set_epoch(1);
        h.enroll_cron_event(&rt, 2, &miner1, &RawBytes::from(vec![0x01, 0x03]))
            .unwrap();
        h.enroll_cron_event(&rt, 4, &miner2, &RawBytes::from(vec![0x02, 0x03]))
            .unwrap();
    ...
    fn event_scheduled_in_past_called_next_round() {
        ...
        h.enroll_cron_event(&rt, 2, &miner_addr, &RawBytes::from(payload.clone()))
            .unwrap();
    ...
    fn skips_invocation_if_miner_has_no_claim() {
        ...
        h.enroll_cron_event(&rt, 2, &miner1, &RawBytes::default())
            .unwrap();
        h.enroll_cron_event(&rt, 2, &miner2, &RawBytes::default())
            .unwrap();
...
mod cron_batch_proof_verifies_tests {
    ...
    fn success_with_one_miner_and_one_confirmed_sector() {
        ...
        h.submit_porep_for_bulk_verify(&rt, MINER_1, info.clone(), true)
            .unwrap();

        let confirmed_sectors = vec![ConfirmedSectorSend {
            miner: MINER_1,
    ...
    fn duplicate_sector_numbers_are_ignored_for_a_miner() {
        ...
        let infos = vec![
            create_basic_seal_info(1),
            create_basic_seal_info(1),
            create_basic_seal_info(2),
        ];
    ...
    fn skips_verify_if_miner_has_no_claim() {
        ...
        h.submit_porep_for_bulk_verify(&rt, MINER_1, info, true)
            .unwrap();
    ...
    fn success_with_multiple_miners_and_multiple_confirmed_sectors_and_assert_expected_power() {
        ...
        h.submit_porep_for_bulk_verify(&rt, miner1, info1.clone(), true)
            .unwrap();
        h.submit_porep_for_bulk_verify(&rt, miner1, info2.clone(), true)
            .unwrap();

        h.submit_porep_for_bulk_verify(&rt, miner2, info3.clone(), true)
            .unwrap();
        h.submit_porep_for_bulk_verify(&rt, miner2, info4.clone(), true)
            .unwrap();

        h.submit_porep_for_bulk_verify(&rt, miner3, info5.clone(), true)
            .unwrap();
        h.submit_porep_for_bulk_verify(&rt, miner3, info6.clone(), true)
            .unwrap();

        h.submit_porep_for_bulk_verify(&rt, miner4, info7.clone(), true)
            .unwrap();
        h.submit_porep_for_bulk_verify(&rt, miner4, info8.clone(), true)
            .unwrap();
...
mod submit_porep_for_bulk_verify_tests {
    ...
    use fil_actors_runtime_v11::shared::HAMT_BIT_WIDTH;
    use fil_actors_runtime_v11::test_utils::{make_piece_cid, make_sealed_cid};
    use fil_actors_runtime_v11::Multimap;
    ...
    fn registers_porep_and_charges_gas() {
        ...
        h.submit_porep_for_bulk_verify(&rt, MINER, info, true)
            .unwrap();
        let st: State = rt.get_state();
        let store = &rt.store;
    ...
    fn aborts_when_too_many_poreps() {
        ...
        for i in 0..MAX_MINER_PROVE_COMMITS_PER_EPOCH {
            h.submit_porep_for_bulk_verify(&rt, MINER, create_basic_seal_info(i), true)
                .unwrap();
        }
